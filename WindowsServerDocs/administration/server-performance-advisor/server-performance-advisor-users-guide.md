...
Questo manuale dell'utente per Microsoft Server Performance Advisor (SPA) fornisce linee guida sull'utilizzo di SPA per identificare i colli di bottiglia delle prestazioni in sistemi distribuiti in vari ruoli server.

SPA consentono con quanto segue:

* Gestire le prestazioni del server e risolvere i problemi di prestazioni di server.

* Fornire indicazioni sulla configurazione comune e problemi di prestazioni e report sui dati.

* Fornire migliori pratice consigli basati sui dati raccolti.
...
La versione originale di Microsoft Server Performance Advisor (SPA) è stata rilasciata nel 2006 2005. Principalmente come destinazione Windows Server 2003, inclusi il sistema operativo e i ruoli del server, ad esempio Internet Information Services (IIS) e active directory. L'obiettivo dello strumento è che consentono di valutare le prestazioni del server e risolvere i problemi di prestazioni di server.
...
### <a href="" id="what-s-new-in-spa-3-1-"></a>Novità in SPA 3.1?

In SPA 3.1 sono state aggiunte le funzionalità e i miglioramenti seguenti:

* Active directory Service Pack di Advisor consente di analizzare le prestazioni generali del ruolo server Servizi di dominio di active directory del server s.
...
* **Ambiente server**
    ...
    SPA è progettato per configurare e gestire facilmente. Si applica agli ambienti server che utilizzano server 1 e 100. SPA non vengono scalati bene se si sta tentando di gestire le prestazioni per più di 100 server. Per ambienti più grandi o più sofisticati, è opportuno utilizzare System Center Operations Manager.
...
**Nota** SPA non include SQL Server o a .NET Framework come parte del pacchetto di installazione SPA. Dopo l'installazione di Microsoft SQL Server 2008 R2 e .NET Framework 4.0, è consigliabile eseguire Windows Update prima di installare l'applicazione a singola PAGINA.
...
### <a href="" id="bkmk-setupspa"></a>Impostazione di SPA

SPA viene assemblato come file con estensione cab che include tutti i file binari per il framework SPA, i cmdlet di Windows PowerShell utilizzati in scenari avanzati, e comprime i seguenti consigli: Core del sistema operativo, Hyper-V, active directory e IIS. Dopo aver estratto il file CAB in una cartella, è necessaria alcuna installazione aggiuntiva. Tuttavia, per eseguire l'applicazione a singola PAGINA, è necessario abilitare la raccolta dati dal server di destinazione come segue:
...
* Assicurarsi che l'account utente utilizzato per eseguire la console SPA dispone di autorizzazioni lettura e scrittura alla cartella condivisa. Erano messe a disposizione Usa questo account per scrivere i log nella cartella.
La console SPA utilizza lo stesso account per leggere i log e importarli nel database.

    **Nota** ETW implementa un buffer circolare per archiviare la traccia e li sposta nella cartella condivisa quando possibile. Se il server è occupato o l'operazione di scrittura è lenta, ETW Elimina le tracce quando il buffer è pieno. È importante che la cartella condivisa si trova in un server con accesso i/o veloce. È consigliabile che ogni server di destinazione disponga di una cartella condivisa per ridurre al minimo la perdita di dati dovuta ai / o file lenta.
...
* Se il server di console e di destinazione si trovano in subnet diverse, è anche necessario impostare il campo dell'indirizzo IP remoto nelle regole del firewall in entrata nel **ambito** impostazioni il **avvisi e registri di prestazioni** pagina, come illustrato di seguito.
...
* Se il server di destinazione non è stato aggiunto a un dominio, abilitare l'impostazione del Registro di sistema seguente: **HKLM\\SOFTWARE\\Microsoft\\Windows\\Currentversion\\Policies\\system\\LocalAccountTokenFilterPolicy**.

**Nota** per impostazione predefinita, SPA scrive i log di diagnostica nella cartella in cui si trova SpaConsole.exe. Se SPA è installato nella cartella programmi, SPA solo essere in grado di scrivere il log quando SpaConsole.exe è stato eseguito come amministratore.

Se si desidera eseguire l'analisi dei dati su computer della console, è necessario eseguire il sistema come amministratore. PIA passa attraverso un percorso di codice diverso durante l'esecuzione per un computer locale, che richiede privilegi di amministratore.

Se si desidera eseguire il pacchetto di Advisor SPA IIS sui server, è necessario abilitare query WMI e traccia ETW per il server IIS. Tale scopo, è possibile abilitare **traccia** sotto il **integrità e diagnostica** servizio ruolo e **Strumenti e script di gestione IIS** in **gli strumenti di gestione** del **Web Server (IIS)** ruolo del server.

 

### <a name="creating-your-first-project"></a>Creazione del primo progetto

Dopo tutto è configurato, è possibile creare il primo progetto di applicazione a singola PAGINA. Come descritto nella sezione precedente, il SISTEMA memorizza tutto ciò che è correlato all'analisi dei dati delle prestazioni all'interno di un database. Ogni progetto di applicazione a singola PAGINA corrisponde a un singolo database. Il **innanzitutto ora utilizza guidata** in modo semplificato i passaggi seguenti.

**Per creare un progetto di applicazione a singola pagina**
...
2.  Per creare un nuovo progetto, fare clic su **File**, quindi fare clic su **Nuovo progetto**. Verrà avviata la prima volta usare guidata. La prima pagina vengono illustrati i passaggi successivi quando si usa la procedura guidata:
...
3.  Fare clic su **Avanti**. Il **creare progetto database** pagina richiederà di immettere il nome dell'istanza di Microsoft SQL Server in cui si desidera creare il database. Ad esempio, se è nello stesso computer della console, è possibile usare **localhost\\&lt;il nome del server SQL&gt;**.

    **Nota** il nome dell'istanza predefinito per un'installazione di SQL Server 2008 R2 Express è SQLExpress. Predefinito in genere per un'istanza di SQL Server 2008 R2 Express è installato nel computer locale, il database verrà impostato su **localhost\\SQLExpress**. Tuttavia, si sia stato modificato durante l'installazione di SQL Server, pertanto è necessario utilizzare il nome dell'istanza SQL Server corretto.
...
4.  Fornire il nome del database. Solo lettere, cifre e caratteri di sottolineatura (\_) sono consentiti come caratteri validi per un nome di database. Suggerimenti per il nome del database SPA ragionevoli sarebbe **SPA**. Se si immette un nome non valido, viene visualizzata un'icona rossa di errore. La descrizione comando associata indica il motivo dell'errore di convalida.
    ...
    **Nota** è importante ricordare il nome del database e il nome dell'istanza server, perché questi sono solo identificatori per il progetto. È necessario fornire queste informazioni se si desidera passare a questo database.

     

5.  Dopo aver specificato il nome dell'istanza server e il nome di database, la prima volta usare guidata genera il percorso del file di database.

6.  Nel **creare database di progetto** pagina, fare clic su **successivo**. La prima volta usare guidata viene creato un database e genera lo schema del database correlate SPA tutti, funzioni e stored procedure nel database. Questo passaggio potrebbe richiedere alcuni secondi a seconda della velocità di hardware e di rete.
...
7.  Quando il passaggio precedente ha esito positivo, viene visualizzato il **provisioning pack advisor** pagina. Elenca tutti i Pack advisor sono disponibili nel computer in uso. SPA analizza automaticamente la cartella denominata **APs** nella directory radice SPA. Elenca il nome completo, versione e l'autore per ogni Service pack di advisor.

    **Nota** per altre informazioni sull'utilizzo il nome completo e la versione in SPA, vedi [gestione Pack di advisor](#bkmk-manageadvisorpacks)
...
8.  Scegliere quali Pack advisor che si desidera eseguire il provisioning nel database del progetto e quindi fare clic su **Avanti**. È possibile fare clic su **Skip** per passare al passaggio successivo senza provisioning qualsiasi Pack di advisor.
    ...
    **Nota** è possibile effettuare il provisioning Pack di advisor ogniqualvolta si usa lo strumento. Per ulteriori informazioni, vedere [Gestione Pack di advisor](#bkmk-manageadvisorpacks).

     

9.  Nel **aggiungere i server** pagina, per ogni server da aggiungere all'elenco di server di destinazione, sono presenti due campi obbligatori da compilare: **Nome del server** e **percorso di condivisione File**.

    **Nota** è inoltre disponibile una **osservazione** campo, che viene utilizzato principalmente per classificare o trovare il server. In casi in cui si dispone di molti server, è possibile importare un file di elenco delimitato da virgole (CSV) che contiene il nome del server, la cartella dei risultati e il campo è facoltativo. Il **osservazione** campo viene usato per descrivere il server e il termine può essere utilizzato per filtrare i server per la raccolta dei dati. Se si siano inizializzando i server tramite il file con estensione csv, un errore di analisi all'interno del file non verrà caricato il server.

     

10. È necessario impostare per abilitare la raccolta di dati erano messe a Disposizione, come descritto in diverse configurazioni [impostazione SPA](#bkmk-setupspa). Il **Aggiungi server** pagina fornisce una funzionalità di configurazione di test per agevolare la risoluzione dei problemi di configurazione. Selezionare la casella di controllo associata al computer e quindi fare clic su **testare la connettività**. SPA tenta di generare un agente di raccolta dati sui server di destinazione e si tenta di importare di che nuovo i risultati nel database. Se tutto è corretto, il **stato** Mostra **passare**. In caso contrario, viene visualizzato un suggerimento che descrive il motivo dell'errore.

11. Ogni server viene aggiunto automaticamente al database, anche se non ha superato il test di configurazione. Per rimuovere server dall'elenco, selezionare il nome del server e fare clic su **rimuovere**.

12. Quando tutto ciò che viene completato, fare clic su **fine** per chiudere la prima volta usare guidata. Se si chiude la prima volta usare guidata prima il completamento, tutti i passaggi precedenti vengono mantenuti e nessuno di essi rollback automaticamente. È necessario apportare manualmente modifiche future.
...
Ogni volta che viene avviata la console SPA, l'ultimo progetto che è stato utilizzato dall'utente corrente viene aperto automaticamente. La finestra principale contiene un elenco di server. Ogni server dispone di quattro proprietà: Nome del server, risultato dell'analisi, lo stato corrente e osservazioni.
...
* **Risultato dell'analisi** per impostazione predefinita, viene illustrato il risultato dell'analisi delle prestazioni più recente esecuzione sul server. Se non è stata qualsiasi analisi delle prestazioni eseguire nel server, verrà visualizzato **alcun report**. Se viene generato un avviso generato dal report, verrà visualizzato **avviso** e il timestamp quando è stato generato il report più recente. Se durante l'analisi più recente sul server è stato trovato alcun problema, verrà visualizzato **OK** e il timestamp.
    ...
    **Nota** se è stata recentemente modificata un'impostazione di sistema, è consigliabile eseguire l'analisi per valutare l'impatto della modifica e ottenere un report di aggiornamento dello stato del sistema. SPA non tiene traccia delle modifiche di configurazione per il sistema sottoposto a test.
...
* **osservazione** descrive il server di destinazione corrente. Ad esempio, è possibile descrivere il server con il ruolo del server (ad esempio, SQL Server) o un percorso (ad esempio, Kent). SPA utilizza il **nome Server** e **osservazione** per cercare e trovare il server corretto. È possibile digitare nella casella di testo di ricerca. Se il **nome Server** oppure **osservazione** colonne contiene la stringa esatta immesso nella casella di ricerca, il server viene visualizzato nell'elenco del server.
...
Per eseguire analisi delle prestazioni nei server di destinazione, selezionare i server che si desidera eseguire l'analisi delle prestazioni su e fare clic su * * eseguire l'analisi * * una finestra di dialogo viene visualizzata e viene chiesto di selezionare i pacchetti di advisor per essere eseguiti nei server. I pacchetti selezionati advisor eseguire simultaneamente. I report generati sono basati sui dati sulle prestazioni raccolti nello stesso periodo di tempo.

**Nota** se si seleziona un server con un'analisi delle prestazioni ricorrente in esecuzione, il **Rimuovi ricorrenza** pulsante consente di annullare la raccolta dati ricorrente. SPA non consente più sessioni di raccolta dati contemporaneamente nello stesso computer.

 

## <a href="" id="bkmk-viewingreports"></a>Visualizzazione dei report
...
È inoltre possibile cercare all'interno di un report. Un numero di report hanno una **trovare** casella di ricerca stringa disponibile per la ricerca di stringhe di testo rapida all'interno del report. Deselezionare la casella di testo, è possibile chiuderla. Per attivare una casella di ricerca (in windows con il testo di ricerca), è possibile utilizzare la combinazione CTRL + F scelta rapida. Il **trovare** casella consente all'utente di specificare una ricerca tra maiuscole e minuscole come appropriato con i **maiuscole/minuscole** opzione.

La figura seguente mostra le **trovare** casella di ricerca con la stringa **Power** sul **Report** scheda.
...
Se l'origine dati per una regola soddisfa determinate condizioni basate sulle impostazioni per la logica e soglia, viene visualizzato un avviso nel **avviso** area. Un avviso include le parti seguenti:
...
* **Singolo valore** una coppia chiave/valore. La chiave è una stringa che rappresenta il significato del valore. Il valore può essere una stringa, un valore numerico o un valore booleano. Ciò viene spesso utilizzato per mostrare informazioni statiche, come architettura di configurazione, ad esempio, la CPU, le dimensioni della memoria totale e la versione del BIOS, che non cambiano nel tempo.

* **elencare valore** in alcuni casi si tratta di una coppia chiave/valore, ma il valore dell'elenco può contenere più campi. Ad esempio, l'attributo della CPU può essere visualizzata in una tabella con più colonne e più righe. Ogni riga rappresenta una CPU e ogni colonna rappresenta un attributo della CPU.
...
Come indicato nelle sezioni precedenti, il SISTEMA si basa su erano messe a Disposizione per raccogliere una traccia ETW, query WMI, i contatori delle prestazioni, le chiavi del Registro di sistema e i file di configurazione per generare il report. È importante comprendere l'origine dati alla base di ogni punto dati nel report. SPA fornisce tali informazioni tramite le descrizioni comandi. È possibile posizionare sulle righe per visualizzare una descrizione dell'origine dati o colonne chiave. Ad esempio, **WMI:Win32\_DisDrive: didascalia** significa che l'origine dati deriva da una query WMI, il nome della classe WMI è Win32\_DiskDrive e la proprietà è **didascalia**.

### <a href="" id="side-by-side-report-"></a>Report side-by-side
...
Se si elimina il **trovare** nella finestra di dialogo sarà possibile riattivarlo in digitando Ctrl + F. Questa finestra di dialogo individua ed evidenzia le stringhe di testo all'interno della sezione corrente.

Nella sezione di notifica, se i due report che vengono confrontati i risultati è un avviso, è elencato nel **avviso** area. In caso contrario, i risultati sono elencati nel **altre notifiche** area. Poiché la chiave per un report side-by-side consiste nell'identificare le differenze tra i report, non viene visualizzate alcun informazioni dettagliate su una regola. Gli utenti potranno selezionare il nome della regola per visualizzare il modulo dettagli regola per ulteriori informazioni sulla regola.
...
1.  Per individuare eventuali anomalie in un certo periodo di tempo per un punto dati, ad esempio, in ogni giorno alle 2:00 i **latenza media richieste** di IIS passa da circa 200 ms a 500 ms.
...
La serie di dati selezionato nella tendenza e grafici cronologici vengono archiviata come una preferenza dell'utente. La prossima volta che la tendenza e un grafico di andamento storico visualizzatore viene aperto per lo stesso pack di advisor, lo stesso set di serie di dati vengono elencate come il valore predefinito.
...
**Nota** report eliminato non può essere annullato.

 

### <a name="exporting-and-importing-reports"></a>Esportazione e importazione di report

I report possono essere esportati in un file XML al trasporto per un'altra console SPA o per posta elettronica a un altro utente. Esportazione del report non comporta l'eliminazione del report. Per esportare il report visualizzato al momento, da **Visualizzatore Report**, fare clic su **azioni**, quindi fare clic su **esportare**. Per esportare più report, dal **Esplora Report**, fare clic su **selezione multipla consentono**, selezionare la casella di selezione più report e quindi fare clic su **esportare**. Ciò consente di esportare i report in formato XML nella directory di destinazione selezionata.

È possibile visualizzare un report esportato in SPA. i report importati non vengono aggiunti al database di SPA. Sono utilizzati principalmente per essere utilizzato come un visualizzatore XML per il report esportato. Non è necessario che il server per il report importato gli stessi Pack advisor installato come la console SPA originale del report esportato.

## <a href="" id="bkmk-manageadvisorpacks"></a>Gestione Pack di advisor


SPA include pacchetti di advisor per il sistema operativo, Hyper-V, active directory e IIS. SPA fornisce un'architettura aperta per lo sviluppo Pack di advisor tramite SQL, è anche possibile per gli sviluppatori non Microsoft compilare le versioni dei pacchetti di advisor. Sono disponibili quattro opzioni per la gestione di un pacchetto di preparazione: effettuare il provisioning, personalizzare, reimpostare o rimuovere.
...
**Effettuare il provisioning di un nuovo pacchetto di advisor**

1.  copiare tutto il contenuto del pacchetto di advisor nel *% SpaRoot %*\\directory APs.
...
2.  Nella finestra principale, fare clic su **configurazione**, quindi fare clic su **configurare Pack di Advisor**. Il **configurare Pack di Advisor** verrà visualizzata la finestra di dialogo.
    ...
    **Nota** questa finestra di dialogo è simile al **provisioning pack advisor** pagina della prima ora utilizza guidata. Viene illustrato un elenco di pacchetti di advisor che sono disponibili per la gestione. Ogni pacchetto di advisor nell'elenco contiene le proprietà, quali nome, versione, versione e autore installati. Nome è il nome completo del pacchetto di preparazione e versione installata è la versione di questo pacchetto di advisor è già stato eseguito il provisioning del progetto. Se il pacchetto di advisor non è disponibile nel database corrente, viene visualizzata la casella di testo versione installata **non è installato**. Il campo versione indica la versione Pack advisor, che si trova sotto la cartella Pack di advisor.

     

3.  Selezionare il pacchetto di advisor nell'elenco. Se il pacchetto di advisor non è stato effettuato o se è presente una versione più recente nella cartella packs advisor rispetto a quello del database, il **provisioning** pulsante è abilitato. Fare clic sui **provisioning** pulsante.

4.  Al termine, il provisioning di **versione installata** campo per il pacchetto di advisor selezionato contiene le informazioni sulla versione nuova.
...
per altre informazioni su come modificare e compilare Pack di advisor, vedere [Guida di sviluppo Server Performance Advisor Pack](server-performance-advisor-pack-development-guide.md).
...
1.  Nel **configurazione** menu, fare clic su **configurare Pack di Advisor**, scegliere il nome del pacchetto di preparazione da modificare e quindi fare clic su **Configura**.
    ...
    **Nota** viene visualizzato un elenco di tutte le regole incluse nel pacchetto di advisor. La casella di controllo a sinistra del nome del Service pack di advisor indica se la regola è attivata. Se una regola è disabilitata, è nascosto da tutti i report.
...
Alcune delle regole non è necessario soglie definite. Ad esempio, il **Keep-Alive HTTP** regola controlla il valore booleano per IIS. Pertanto, l'elenco di valori di soglia può essere vuota. In caso contrario, viene elencato il tutte le soglie utilizzate dalla regola corrente. Una descrizione dettagliata sull'utilizzo di una soglia della regola è inclusa come parte della descrizione.

Se il **dettagli regola** form viene avviato dalle **configurazione** menu, l'elenco di soglia con tre colonne: nome impostazione originale e impostazione di modifica. Se viene avviata da un report singolo o side-by-side, i valori di soglia utilizzati nel report vengono anche essere inclusi. Gli utenti possono modificare i valori di soglia correnti, modificando il valore in **modificare l'impostazione** colonna e quindi scegliendo **salvare** per salvare le modifiche al database.

Tutte le modifiche apportate ai valori di soglia è solo applicate ai report che vengono generati dopo le modifiche. I report esistenti non sono interessati da queste modifiche.

### <a name="sharing-thresholds"></a>Condivisione delle soglie

Se si gestiscono i server in situazioni simili, è possibile usare lo stesso set di valori di soglia. È possibile esportare e importare le soglie per un pacchetto di advisor specifico utilizzando il **configurazione** menu. È possibile selezionare il pacchetto di advisor specifico e quindi fare clic su **Configura**. Il file esportato soglia è in formato XML.

Quando si importa una soglia, SPA convalida il formato di file XML e verifica che il file corrisponda al pack advisor selezionato. Se l'operazione viene completata, SPA Importa tutti i valori dal file di soglia nel database di progetto corrente. Simile a quello precedente la modifica delle soglie, tutte le modifiche dei valori di soglia hanno effetto solo su report generati in futuro. I report esistenti non sono interessati.

### <a name="enable-or-disable-rules"></a>Abilitare o disabilitare le regole

Può essere attivata o disattivata da una regola di **Dettagli regola** form. È necessario fare clic su **salvare** per rendere persistenti le modifiche apportate. Se una regola è disattivata, è a non essere visualizzato in uno dei report. Ma la logica di business sottostante viene attivata durante la generazione di report, in modo che quando si sceglie di abilitare nuovamente la regola, viene visualizzato nei report nuovamente.

### <a name="reset-advisor-packs-to-original-state"></a>reimpostazione Pack di advisor allo stato originale
...
per altre informazioni su come modificare un pacchetto di provisioning advisor, vedere la [Guida di sviluppo Server Performance Advisor Pack](server-performance-advisor-pack-development-guide.md).

Se si desidera eseguire il rollback delle modifiche apportate in un pacchetto di provisioning advisor, è possibile scegliere di reimpostare il pacchetto di advisor. Questa registrazione sovrascriva tutti gli script SQL che sono correlati al pack di advisor e reimpostare tutti i valori di soglia predefinito. In questo modo tutti i report esistenti.

reimpostazione pack di advisor può essere eseguita tramite il **configurare Pack di Advisor** form. È necessario selezionare il pacchetto di advisor reimpostare e quindi fare clic su **reimpostare**.

### <a name="remove-advisor-packs"></a>rimuovere Pack di advisor

Quando un pacchetto di advisor non è più necessario, gli utenti possono rimuoverlo dal database. Quando si rimuove il pacchetto di advisor, tutti gli aspetti pack di advisor dal database, incluse le regole e soglie, tutti gli script SQL e tutti i report. Nessuna delle azioni può essere eseguito il rollback.

è possibile rimuovere il pacchetto di advisor tramite **configurare Pack di Advisor** form. È necessario selezionare il pacchetto di preparazione da rimuovere e quindi fare clic su **Deprovision**.
...
Se sono presenti solo le modifiche per la logica di business e non segnalare le modifiche di metadati per un pacchetto di advisor, è opportuno assegnare un nuovo numero di versione, ad esempio da 1.0 a 2.0. Se è presente una modifica dei metadati di report, il Service pack di advisor devono avere un nome diverso. Ad esempio, Microsoft.ServerPerformanceAdvisor.IIS.V1 potrebbe essere modificato in Microsoft.ServerPerformanceAdvisor.IIS.V2.

Se una versione più recente del pacchetto di advisor esiste, l'elenco nel **configurare Pack di Advisor** form verrà inserito automaticamente il **versione** colonna con la versione più recente del pacchetto di advisor. È possibile selezionare il pacchetto di advisor e quindi scegliere il **reimpostare**. Gli aggiornamenti di Service pack di advisor con le nuove regole business e le soglie. Vengono mantenuti tutti i report per questo Service pack di advisor.
...
**Per aggiungere o rimuovere i server o modificare informazioni del server**
...
3.  Immettere altre informazioni sul server nel **osservazione** colonna per ogni server.

    **Nota** questo campo viene utilizzato un formato di testo libero, in modo che è possibile usarlo come un campo di descrizione. Utilizzare questo campo per contrassegnare i server in modo sono reperibili facilmente nella finestra principale o del gruppo di server, ad esempio, dal ruolo del server o posizione.

     

4.  Se si desidera utilizzare SPA con un numero elevato di server, SPA supporta un formato con valori delimitati da virgole (CSV) per l'importazione. Il file deve contenere almeno due campi: **Server** e **percorso della condivisione File**. Il terzo campo **osservazione** è facoltativo, ma è consigliabile per organizzare i server. È inoltre possibile esportare l'elenco dei server in un file CSV per determinare il formato appropriato o eseguire il backup della configurazione del server.

### <a name="searching-and-filtering"></a>Ricerca e filtro

Se si gestiscono più server, SPA fornisce il supporto di base per trovare rapidamente i server nella finestra principale. È possibile fare clic sulla colonna intestazione per ordinare in base al nome del server, risultati dell'analisi, lo stato corrente delle attività o la sezione Osservazioni. È inoltre possibile scegliere di utilizzare la funzionalità di ricerca. Nell'angolo superiore destro della finestra principale, è possibile digitare una stringa da cercare. Il **server di destinazione** elenco nella finestra principale Usa la stringa per filtrare i server e per visualizzare solo i server con i campi nome o l'osservazione che contiene la stringa di ricerca.

La figura seguente mostra come la stringa **delL** corrisponde al server con la stringa **delL** o server con **osservazione** campo contenente **delL**.
...
1.  Da un prompt dei comandi di Windows PowerShell con privilegi elevati, digitare **Registerspacmdlets**. Il **registrare i cmdlet SPA correttamente** messaggio viene visualizzato.

2.  Eseguire **PowerShell.cmd SPA**. Se si passa il percorso al file di script Windows PowerShell, eseguire gli script automaticamente. In caso contrario, viene aperto un prompt dei comandi di Windows PowerShell, è possibile eseguire i cmdlet di SPA Windows PowerShell.

Nella tabella seguente vengono descritti i cmdlet di SPA Windows PowerShell:

| Nome del cmdlet | Parametri | Descrizione |
| ------ | ------- | ------ |
| Inizio SpaAnalysis | **-ServerName** nome del server di destinazione.<br>**-AdvisorPackName** nome completo del pacchetto di preparazione mettere in coda nel server. Quando più di un pacchetto è pianificato per l'esecuzione nello stesso momento, il valore del parametro deve essere formattato come AP1name, AP2name.<br>**-Durata** durata per la raccolta dei dati.<br>**-Credenziale** credenziali utente per l'account che esegue la raccolta dei dati nel server di destinazione.<br>**-SqlInstanceName** nome dell'istanza di SQL Server.<br>**-SqlDatabaseName** nome del database di progetto di applicazione a singola pagina. | Avvia una sessione di raccolta dati SPA nel server specificato. |
| Stop-SpaAnalysis | **-SqlInstanceName** nome dell'istanza di SQL Server.<br>**-SqlDatabaseName** nome del database di progetto di applicazione a singola pagina.<br>**-ServerName** nome del server di destinazione. | Tenta di arrestare una sessione SPA in esecuzione. Se una sessione è già completa, viene restituito senza eseguire alcuna operazione. |
| Get-SpaServer | **-SqlInstanceName** nome dell'istanza di SQL Server.<br>**-SqlDatabaseName** nome del database di progetto di applicazione a singola pagina. | Ottiene l'elenco di server nel database. Restituisce un elenco di oggetti, tra cui queste proprietà: Nome, stato, condivisione file e osservazioni. |
| Get-SpaAdvisorPacks | **-SqlInstanceName** nome dell'istanza di SQL Server<br>**-SqlDatabaseName** nome del database di progetto di applicazione a singola pagina | Ottiene l'elenco di Service pack di advisor nel database. Restituisce un elenco di oggetti, tra cui queste proprietà: Name, DisplayName, autore e versione. |

Windows PowerShell offre la possibilità di passare le credenziali tramite i file crittografati per abilitare gli scenari di automazione. Per altre informazioni sull'uso di file crittografati per passare le credenziali per un cmdlet, vedere [crea Windows PowerShell script tale accetta credenziali](https://technet.microsoft.com/magazine/ff714574.aspx).
...
ad esempio, il comando T-SQL seguente fornisce un elenco primi 10 server dalla CPU per l'intervallo di tempo che coprono degli ultimi tre giorni:
...
**Nota** SQL Server 2008 R2 Express è un limite di 10 GB del database. Utilizzando più progetti, è possibile utilizzare uno o più database di SQL Server e restare sotto il limite di 10 GB SQL Server 2008 R2 Express.
...
* **Critico** Registra solo errori ed eccezioni che non possono essere gestite dalla console di SPA. Questi errori causano la console SPA in arresto anomalo del sistema. I registri di forniscono le informazioni di contesto per tali errori.

Per impostazione predefinita, il livello di registrazione è avviso, il che significa che SPA registra solo errori ed eccezioni che si verificano nell'applicazione a singola pagina. Il livello di registrazione può essere modificato modificando il **SpaConsole.exe.config** file SpaConsole.exe si trova nella stessa cartella. Tutti i log vengono scritti i file di log. txt nella stessa cartella.
...
per altre informazioni sulle funzionalità di debug in SPA, vedi il [Guida di sviluppo Server Performance Advisor Pack](server-performance-advisor-pack-development-guide.md).
...
### <a name="clean-up-the-database"></a>pulizia del database
...
Tutti i dati raccolti da SPA vengono archiviati nei database del progetto. A causa della natura di alcune delle tracce ETW, SPA potrebbe raccogliere informazioni riservate in grado di importanza elevata business. È necessario essere consapevoli del rischio potenziale associato condividono l'accesso ai database di progetto di applicazione a singola PAGINA. File di log temporanei vengono salvati in cartelle condivise che vengono specificate in ogni computer di destinazione. Anche se tenta di SPA per eliminare tali log temporaneo, i file al termine dell'importazione di dati, non c'è garanzia che i file di log vengono sempre essere eliminati. È necessario assicurarsi che le cartelle condivise si trovano in percorsi sicuri.
...
Quando si prova a eseguire SPAConsole.exe per la prima volta, se non è installato .NET Framework, l'applicazione non avviare o scrivere un file di log. Verificare che un compatible.NET che Framework sia installato e funzionante in modo corretto prima di avviare il SISTEMA.

### <a name="locating-log-information"></a>Individuazione delle informazioni di log

SPA archivia informazioni sull'errore nel file di log. txt nella cartella SPA. informazioni sullo stack di chiamate e messaggi di errore dettagliati vengono scritti in questa cartella. Se si verificano errori che richiedono ulteriori informazioni su come interpretare, è possibile aprire il file di log. txt per visualizzare i dettagli dell'errore.
...
Se si usa SQL Server Express, il database utente è limitato a 10 GB, ma il file di registro corrispondenti può superare i 70 GB. Per questi motivi, è consigliabile più di 100 GB di spazio libero su disco per SQL Server Express. Questo spazio su disco dovrebbe essere sufficiente per archiviare circa i report da 20.000 a 30.000. Questo file di log è denominato SPADB\_log. ldf e viene trovato in **% Program Files %\\Microsoft SQL Server\\MSSQL10. SQLEXPRESS\\MSSQL\\dati**.
...
Se si verifica una non è possibile creare Set di raccolta dati erano messe a disposizione nel messaggio di server di destinazione, assicurarsi di eseguire le operazioni seguenti:

* Assicurarsi che il **gli avvisi e registri di prestazioni** servizio è in esecuzione

* L'impostazione di sicurezza **accesso alla rete: Non consentire l'archiviazione delle password e credenziali per l'autenticazione di rete** è disabilitato. L'impostazione di sicurezza deve essere disabilitato perché SPA deve utilizzare le credenziali utente per creare il Set di raccolta dati nel server di destinazione.

### <a href="" id="running-spa-against-the-console-"></a>In esecuzione il sistema per la console

Se il server di destinazione è quello utilizzato per la console SPA, erano messe a Disposizione passa attraverso un canale diverso. Anche se l'account utente è in esecuzione la console SPA con privilegi di amministratore, erano messe a disposizione avrà esito negativo. Se in un server di destinazione è installata la console SPA, gli utenti devono avviare SPA come amministratore per assicurarsi che l'attività di analisi delle prestazioni è possibile eseguire nella console.
...
Quando si esegue il SPAConsole.exe e si utilizza una raccolta di dati ricorrenti (ad esempio, una raccolta oraria), il server che esegue il SPAConsole.exe non deve essere in modalità di risparmio energia in modo che è possibile sospendere. SPA non verifica la presenza di criteri di risparmio. Questa attività sospesa potrebbe interferire con la raccolta dei dati regolare ricorrente.

### <a name="lost-etw-events"></a>Eventi ETW persi

Per creare impatto minimo sulle prestazioni nei server di destinazione, erano messe a Disposizione è progettata per funzionare con priorità bassa, mentre è la raccolta di informazioni sulle prestazioni. Se il server di destinazione è occupato, erano messe a disposizione possono eliminare alcune delle attività di raccolta dati per produrre alle attività ad alta priorità che sono in esecuzione nei server di destinazione. È consigliabile impostare la cartella condivisa in cui gli eventi scritto in un disco conflitto t l con s il carico di lavoro i/o o un'unità più veloce, ad esempio un'unità SSD. Quando gli eventi vengono eliminati, poiché gli eventi persi possono influire sull'affidabilità delle metriche delle prestazioni generati vengono segnalati nella visualizzazione report.

Alcuni problemi di autorizzazione potrebbero causare anche del Registro di sistema o le query WMI vengono ignorate. Tuttavia, ciò è molto meno probabile che risultino più perdita di eventi ETW. Di conseguenza, i risultati di set di agenti di raccolta dati non contengano tutti i valori richiesti. È necessario assicurarsi che la situazione viene gestita dagli script T-SQL per tutti i pacchetti di advisor. Se i dati non esiste nel risultato della raccolta dati, viene contrassegnato come non dati nei report.
...
Se i risultati di agente di raccolta dati contengono dati incompleti diverso da una perdita di traccia ETW e lo sviluppatore di Service pack di advisor ha aggiunto il supporto per la notifica di perdita di eventi ETW, una barra informazioni viene visualizzata sopra il singolo report per notificare all'utente il potenzialmente report incoerenti causato da una perdita di dati. informazioni di perdita di dati dettagliati sono reperibile nel file di log. txt.
