...
|[/ AnswerClients: {tutti & #124; Nota & #124; Nessuno}]|Specifica che i client risponderà a questo server. Se si imposta questo valore su **noto**, deve essere pre-installazione di un computer in servizi di dominio active directory (AD DS) prima che verrà restituita una risposta dal server di servizi di distribuzione Windows.|
...
|[/AllowN12forNewClients:{Yes &#124; No}]|per Windows Server 2008, specifica che i client sconosciuti non saranno necessario premere il tasto F12 per inizializzare un avvio di rete. I client riceveranno il programma di avvio specificato per il computer oppure, se non specificato, il programma di avvio specificato per l'architettura.<br /><br />per Windows Server 2008 R2, questa opzione è stata sostituita con il comando seguente: wdsutil /Set-Server /pxepromptpolicy /New:|
...
|[/ NewMachineNamingPolicy:<Policy>]|Specifica il formato da utilizzare durante la generazione di nomi di computer per i client. Per informazioni sul formato da usare per <policy>, fare doppio clic su server nello snap-in mmc, fare clic su **delle proprietà**e visualizzare il **servizi directory** scheda. Ad esempio, **/NewMachineNamingPolicy: % 61Username % #**.|
