...
-   Per la sintassi sull'uso di certutil con un verbo specifico, eseguire **certreq**  *\<verb >* **-?**
...
```
Per richiedere un certificato specificando l'attributo SAN, vedere la procedura dettagliata nell'articolo 931351 della Microsoft Knowledge base [come aggiungere un nome alternativo del soggetto a un certificato LDAP sicuro nell'argomento](https://support.microsoft.com/kb/931351) "come utilizzare l'utilità Certreq. exe per creare e inviare un richiesta di certificato che include una sezione SAN.
...
-   Se si usa-config-anziché-config CAComputerName\CAName, l'operazione viene elaborata usando l'autorità di certificazione predefinita.
...
|---|----------|-----|-------|
|Subject|Diverse applicazioni si basano sulle informazioni sul soggetto in un certificato. Pertanto, è consigliabile specificare un valore per questa chiave. Se l'oggetto non è impostato qui, è consigliabile includere un nome soggetto come parte dell'estensione del certificato del nome alternativo del soggetto.|Valori stringa del nome distinto relativo|Subject = "CN = computer1. contoso. com" Subject = "CN = John Smith, CN = Users, DC = contoso, DC = com"|
...
|-Username  *\<nomeutente >*|Utilizzato con i servizi Web di registrazione certificati. È possibile sostituire  *\<username >* con il nome Sam o dominio\utente. Questa opzione è utilizzabile con l'opzione-p.|
|-p  *\<password >*|Utilizzato con i servizi Web di registrazione certificati. Sostituire la password > con la password dell'utente effettivo.  *\<* Questa opzione è utilizzabile con l'opzione-UserName.|
...
|-RenewOnBehalfOf|Inviare un rinnovo per conto dell'oggetto identificato nel certificato di firma. Imposta CR_IN_ROBO quando si chiama [ICertRequest:: Submit](https://technet.microsoft.com/subscriptions/aa385040.aspx)|
...
|Formati|Descrizione|
|-------|-----------|
|RequestFileIn|Nome file di input con codifica Base64 o binario: La richiesta di certificato PKCS #10, la richiesta di certificato CMS, la richiesta di rinnovo del certificato PKCS #7, il certificato X. 509 da certificare o la richiesta di certificato in formato tag KeyGen.|
