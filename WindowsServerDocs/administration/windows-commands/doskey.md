...
|[/insert | /overstrike]|Specifica se inserire o sovrascrivere il testo durante la digitazione. Se si utilizza **/inserire**, nuovo testo digitato in una riga viene inserito nel mezzo del testo esistente. Se si utilizza **/overstrike**, nuovo testo sostituisce il testo esistente. L'impostazione predefinita è **/overstrike**.|
...
-   Creazione di una macro
    ...
    |$B o $b|Invia l'output (macro) a un comando. Questi caratteri speciali sono equivalenti a utilizzare la barra verticale (* *|*) nella riga di comando.|
    ...
    |$*|Rappresenta tutte le informazioni della riga di comando che si desidera specificare quando si digita il nome della macro. Il carattere speciale **$ \*** è un parametro sostituibile simile ai parametri batch **$1** tramite **$9**, con uno importanti differenza: tutti i valori digitati nella riga di comando dopo il nome della macro viene sostituito con il **$ \*** nella macro.|
-   Esecuzione di un **doskey** (macro)

    Per eseguire una macro, digitare il nome della macro al prompt dei comandi, a partire dalla posizione del primo. Se la macro è stata definita con **$ \*** o uno dei parametri di batch **$1** tramite **$9**, usare uno spazio per separare i parametri. Non è possibile eseguire un **doskey** (macro) da un file batch.
