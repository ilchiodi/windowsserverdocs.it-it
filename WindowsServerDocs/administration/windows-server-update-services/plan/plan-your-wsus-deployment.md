...
description: 'Argomento Windows Server Update Service (WSUS): Panoramica del processo di pianificazione della distribuzione con collegamenti agli argomenti correlati'
...
ms.locfileid: "71369844"
---
# <a name="plan-your-wsus-deployment"></a>Pianificare la distribuzione di WSUS

>Si applica a: Windows Server (canale semestrale), Windows Server 2016, Windows Server 2012 R2, Windows Server 2012

Il passaggio iniziale nella distribuzione di Windows Server Update Services (WSUS) prevede l'assunzione di decisioni importanti come, ad esempio, stabilire lo scenario di distribuzione WSUS, scegliere la topologia di rete e comprendere i requisiti di sistema. Nell'elenco di controllo seguente vengono riepilogati i passaggi necessari per la preparazione della distribuzione.

|Attività|Descrizione|
|----|--------|
|[1,1. Esaminare le considerazioni e i requisiti di sistema @ no__t-0|Esaminare l'elenco di considerazioni e requisiti di sistema per verificare di disporre di tutto l'hardware e il software necessario per la distribuzione di Windows Server Update Services.|
|[1,2. Scegliere uno scenario di distribuzione WSUS @ no__t-0|Decidere lo scenario di distribuzione WSUS da utilizzare.|
|[1,3. Scegliere una strategia di archiviazione WSUS @ no__t-0|Decidere la strategia di archiviazione WSUS più adatta alla distribuzione.|
|[1,4. Scegliere le lingue degli aggiornamenti WSUS @ no__t-0|Decidere le lingue degli aggiornamenti WSUS da installare.|
|[1,5. Pianificare i gruppi di computer WSUS @ no__t-0|Pianificare l'approccio dei gruppi di computer WSUS utilizzati per la distribuzione.|
|[1,6. Pianificare le considerazioni sulle prestazioni di WSUS: Servizio trasferimento intelligente in background @ no__t-0|Pianificare una struttura WSUS per le prestazioni ottimizzate.|
|[1,7. Impostazioni Aggiornamenti automatici del piano @ no__t-0|Pianificare come configurare le impostazioni degli aggiornamenti automatici per lo scenario in uso.|
...
-   I requisiti hardware del server per abilitare il ruolo WSUS sono associati ai requisiti hardware. I requisiti hardware minimi per WSUS sono:
    ...
    -   **Processore** processore x64 a 1,4 gigahertz (GHz) (consigliato 2 GHz o superiore)

    -   **Memoria** WSUS richiede 2 GB di RAM aggiuntivi rispetto a quanto richiesto dal server e da tutti gli altri servizi o software.

    -   **Spazio su disco disponibile:** 10 GB (si consiglia 40 GB o superiore)

    -   **Scheda di rete:** 100 megabit al secondo (Mbps) o superiore
...
2. Le edizioni client di Windows non saranno in grado di installare .NET 3.5 su richiesta dal Web. Le stesse considerazioni relative ai ruoli del server si applicano a .NET 3.5.
   ...
   > [!NOTE]
   > La configurazione di un'origine dell'installazione di funzionalità su richiesta non implica WSUS. Per informazioni su come configurare le funzionalità, vedere [Configurare Funzionalità su richiesta in Windows Server](https://technet.microsoft.com/library/jj127275.aspx).

3. I dispositivi aziendali che eseguono Windows 10, versione 1709 o versione 1803, non possono installare funzionalità su richiesta direttamente da WSUS. Per installare funzionalità su richiesta, [creare un file di funzionalità (archivio affiancato)](https://docs.microsoft.com/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/jj127275%28v=ws.11%29#create-a-feature-file-or-side-by-side-store) o ottenere il pacchetto di funzionalità su richiesta da una delle origini seguenti:
   - [Volume Licensing Service Center](https://www.microsoft.com/licensing/servicecenter) (VLSC): è necessario l'accesso VL
   - Portale OEM: è necessario l'accesso OEM
   - Download MSDN: l'abbonamento MSDN è obbligatorio

     I pacchetti di funzionalità su richiesta ottenuti singolarmente possono essere installati usando le [Opzioni della riga di comando DISM](https://docs.microsoft.com/windows-hardware/manufacture/desktop/dism-operating-system-package-servicing-command-line-options).
...
3.  Il server di database deve trovarsi nello stesso dominio Active Directory del server WSUS o deve avere una relazione di trust con il dominio Active Directory del server WSUS.

4.  Il server WSUS e il server di database devono trovarsi nello stesso fuso orario o essere sincronizzati con la stessa origine Coordinated Universal Time (ora di Greenwich).
...
La distribuzione WSUS di base è costituita da un server all'interno del firewall aziendale che serve i computer client in una rete Intranet privata. Il server WSUS si connette a Microsoft Update per scaricare gli aggiornamenti. Questa operazione è denominata  *sincronizzazione*. Durante la sincronizzazione, WSUS determina se sono stati resi disponibili aggiornamenti dall'ultima sincronizzazione effettuata. In caso di prima sincronizzazione, tutti gli aggiornamenti sono resi disponibili per il download.
...
Per impostazione predefinita, per ottenere gli aggiornamenti da Microsoft il server WSUS usa la porta 80 per il protocollo HTTP e la porta 443 per il protocollo HTTPS. Se è presente un firewall aziendale tra la rete e Internet, è necessario aprire le porte sul server che comunica direttamente con Microsoft Update. Se si prevede di utilizzare porte personalizzate per la comunicazione, è necessario aprire tali porte. È possibile configurare più server WSUS da sincronizzare con un server WSUS padre. Per impostazione predefinita, per fornire gli aggiornamenti alle workstation client il server WSUS usa la porta 8530 per il protocollo HTTP e la porta 8531 per il protocollo HTTPS.

### <a name="multiple-wsus-servers"></a>Più server WSUS
Gli amministratori possono distribuire più server che eseguono WSUS che sincronizzano tutti i contenuti all'interno della Intranet dell'organizzazione. È possibile esporre un solo server a Internet, che è l'unico server che Scarica gli aggiornamenti da Microsoft Update. Questo server è configurato come server upstream l'origine a cui vengono sincronizzati i server downstream. Quando applicabile, i server possono essere situati in una rete geograficamente dispersa per fornire la massima connettività a tutti i computer client.
...
> [!NOTE] 
> Si consiglia di non creare una gerarchia di server WSUS profonda più di tre livelli. La presenza dei singoli livelli comporta l'aggiunta di tempo per propagare gli aggiornamenti nei server connessi. Sebbene non esistano limiti teorici a una gerarchia, solo le distribuzioni con una gerarchia di cinque livelli di profondità sono state testate da Microsoft.
>
> Inoltre, i server downstream devono essere della stessa versione o di una versione precedente di WSUS come origine di sincronizzazione del server upstream.
...
Se la rete include utenti mobili che effettuano l'accesso da percorsi diversi, è possibile configurare Windows Server Update Services per consentire agli utenti in roaming di aggiornare i propri computer client dal server WSUS geograficamente più vicino. Ad esempio, è possibile distribuire un server WSUS ogni area e usare una subnet DNS diversa per ogni area. Tutti i computer client possono essere indirizzati allo stesso server WSUS, che viene risolto in ogni subnet nel server WSUS fisico più vicino.
...
> [!NOTE]
> Il database interno di Windows (WID) è stato introdotto in Windows Server 2008.
...
-   [Eseguire il backup e il ripristino dei dati WSUS ed eseguire il backup del server](https://technet.microsoft.com/library/dd939904(WS.10).aspx)
...
Per archiviare tutti gli aggiornamenti, è necessario che il server disponga di sufficiente spazio su disco. come minimo, WSUS richiede 20 GB per archiviare gli aggiornamenti in locale; Tuttavia, si consigliano 30 GB in base alle variabili testate.
...
La pagina **Scelta lingue** della configurazione guidata WSUS consente di ricevere gli aggiornamenti in tutte le lingue o da un sottoinsieme di lingue. la selezione di un subset di lingue consente di risparmiare spazio su disco, ma è importante scegliere tutte le lingue necessarie per tutti i server downstream e i computer client di un server WSUS.

Di seguito sono riportate alcune note importanti sul linguaggio di aggiornamento che è necessario tenere presente prima di configurare questa opzione:
...
> [!NOTE]
> Se un server WSUS viene eseguito in modalità di replica, non è possibile creare gruppi di computer. Tutti i gruppi di computer necessari per i computer client del server di replica devono essere creati nel server WSUS radice della gerarchia. Per ulteriori informazioni sulla modalità di replica, vedere Gestione dei server di replica WSUS [gestione dei server di replica](https://technet.microsoft.com/library/dd939893(WS.10).aspx) WSUS nella Guida operativa di WSUS 3,0 SP2.
...
Nell'esempio di testo seguente del riquadro Gerarchia della console Update Services, per un server WSUS denominato WSUS-01 sono stati aggiunti gruppi di computer denominati computer desktop e server al gruppo predefinito **tutti i computer** . Entrambi i computer desktop e i gruppi di server sono allo stesso livello gerarchico.
...
In questo esempio, il gruppo due livelli sotto il ramo computer desktop (desktop L2) ha una priorità più alta rispetto al gruppo un livello sotto il ramo Server (server L1). Di conseguenza, per un computer appartenente sia al gruppo Desktop-L2 che al gruppo Server-L1, tutte le azioni per il gruppo Desktop-L2 hanno la priorità sulle azioni specificate per il gruppo Server-L1.
...
Prima di distribuire WSUS è necessario pianificare attentamente alcune aree, in modo da ottimizzare le prestazioni. Le principali aree sono:
...
Se si distribuisce una gerarchia di server WSUS connessi, è preferibile non annidarli in profondità. Se si abilitano i download posticipati e un server downstream richiede un aggiornamento che non è approvato nel server upstream, la richiesta del server downstream forza un download nel server upstream. Il server downstream scarica l'aggiornamento nel corso di una sincronizzazione successiva. In una gerarchia di server WSUS profonda, possono verificarsi ritardi quando gli aggiornamenti vengono richiesti, scaricati e passati alla gerarchia. 'Per impostazione predefinita, i download differiti sono abilitati quando i file sono archiviati a livello locale. È possibile modificare manualmente questa opzione.
...
1.  Utilizzare l'opzione di limitazione in BITS (Servizio trasferimento intelligente in background). Le limitazioni della larghezza di banda in BITS possono essere controllate in base all'orario, ma si applicano a tutte le applicazioni che utilizzano BITS. Per informazioni su come controllare la limitazione delle richieste BITS, vedere [criteri di gruppo](https://msdn.microsoft.com/library/windows/desktop/aa362844(v=vs.85).aspx).
