...
description: Argomento di Windows Server Update Service (WSUS) - configurare WSUS è passaggio due per un processo in quattro passaggi per la distribuzione di WSUS
...
ms.locfileid: "59820482"
---
# <a name="step-2-configure-wsus"></a>Passaggio 2: Configurare WSUS

>Si applica a: Windows Server (canale semestrale), Windows Server 2016, Windows Server 2012 R2, Windows Server 2012

Una volta completata l'installazione del ruolo del server WSUS (Windows Server Update Services) nel server, è necessario configurarlo correttamente. Il seguente elenco riassume i passaggi necessari per eseguire la configurazione iniziale per il server WSUS.

|Attività|Descrizione|
|----|--------|
|[2.1. Configurare le connessioni di rete](2-configure-wsus.md#BKM_ConfigurenetworkConnections)|Configurare la rete di cluster tramite la Configurazione guidata rete.|
|[2.2. Configurare WSUS tramite la configurazione guidata WSUS](2-configure-wsus.md#BKM_ConfigureWSUSusingConfigurationWizard)|Usare la Configurazione guidata WSUS per definire la configurazione di base di WSUS.|
|[2.3. Configurare gruppi di computer WSUS](2-configure-wsus.md#BKMK_ConfigcomputerGroups)|Creare gruppi di computer nella console di amministrazione di WSUS per gestire gli aggiornamenti all'interno dell'organizzazione.|
|[2.4. Configurare gli aggiornamenti client](2-configure-wsus.md#BKM_ConfigureClientUpdates)|Specificare come e quando applicare gli aggiornamenti automatici ai computer client.|
|[2.5. Proteggere WSUS con il protocollo Secure Sockets Layer](2-configure-wsus.md#bkmk_2.5.ConfigSSL)|Configurare il protocollo SSL (Secure Sockets Layer) per la protezione di Windows Server Update Services (WSUS).|

## <a name="BKM_ConfigurenetworkConnections"></a>2.1. Configurare le connessioni di rete
...
Per impostazione predefinita, WSUS è configurato per l'utilizzo di Microsoft Update come origine degli aggiornamenti. Se si dispone di un server proxy nella rete, è possibile configurare WSUS per l'uso del server proxy. Se è presente un firewall aziendale tra WSUS e Internet, potrebbe essere necessario configurare il firewall per assicurarsi che WSUS di scaricare gli aggiornamenti.
...
-   **Proxy** se è necessario usare un server proxy per accedere a Internet di WSUS, è necessario configurare le impostazioni proxy nel server WSUS.

-   **Firewall** se WSUS è posizionato dietro un firewall aziendale, esistono alcuni passaggi aggiuntivi che devono essere eseguite nel dispositivo periferico per consentire il traffico di Windows Server Update Services corretto.

### <a name="211-connection-from-the-wsus-server-to-the-internet"></a>2.1.1. Connessione dal server WSUS a Internet
Se è presente un firewall aziendale tra WSUS e Internet, potrebbe essere necessario configurare firewall per consentire a che WSUS di scaricare gli aggiornamenti. Per ottenere gli aggiornamenti da Microsoft Update, il server WSUS usa la porta 443 per il protocollo HTTPS. Sebbene la maggior parte dei firewall aziendali consenta questo tipo di traffico, esistono alcune aziende limitano l'accesso a Internet dal server a causa dei criteri di sicurezza della società. Se l'azienda limita l'accesso, è necessario ottenere l'autorizzazione per consentire l'accesso a Internet da WSUS per l'elenco di URL seguente:
...
3.  Aprire un prompt dei comandi (Cmd.exe) come amministratore. Per aprire un prompt dei comandi come amministratore, fare clic su **Start**. Nelle **Inizia ricerca**, digitare **prompt dei comandi**. nella parte superiore del menu start, fare doppio clic su **prompt dei comandi**, quindi fare clic su **Esegui come amministratore**. Se il **User Account Control** viene visualizzata la finestra di dialogo immettere le credenziali appropriate (se richiesto), verificare che l'azione visualizzata sia quella desiderato e quindi fare clic su **continua**.

4.  Nella finestra del prompt dei comandi, passare alla cartella c:\Programmi\Microsoft Files\Update Services\Tools. Digitare il comando seguente:

    **WSUSUTIL ConfigureSSlproxy [< proxy_server proxy_port >]-abilitare**, dove:
...
4.  Usare le opzioni seguenti per modificare la configurazione del server proxy esistente:
    ...
    4.  Se il server proxy richiede l'utilizzo di un account utente specifico, selezionare la **usare le credenziali dell'utente per connettersi al server proxy** casella di controllo. digitare il nome richiesto all'utente, dominio e password nelle caselle di testo corrispondenti.
...
## <a name="BKM_ConfigureWSUSusingConfigurationWizard"></a>2.2. Configurare WSUS tramite la Configurazione guidata WSUS
...
1.  Nel riquadro di spostamento di Server Manager scegliere **Dashboard**,  **Strumenti**, quindi fare clic su **Windows Server Update Services**.
    ...
    > [!NOTE]
    > Se il **completamento installazione WSUS** verrà visualizzata la finestra di dialogo, fare clic su **eseguire**. Nel **completamento installazione WSUS** finestra di dialogo, fare clic su **Chiudi** quando l'installazione venga completata.
...
3.  Leggere le istruzioni nella pagina **Partecipa al programma Analisi utilizzo Microsoft Update** e valutare se prendervi parte. Se si vuole partecipare al programma, lasciare la selezione predefinita, oppure deselezionare la casella di controllo e quindi fare clic su **successivo**.
...
4.  Nella pagina **Scelta server upstream** sono disponibili due opzioni:
    ...
    2.  Sincronizzare da un altro server Windows Server Update Services
        ...
        -   Se si sceglie di eseguire la sincronizzazione da un altro server WSUS, specificare il nome del server e la porta su cui comunicherà il server con il server upstream.
        ...
        -   Se si tratta di un server di replica, selezionare la **si tratta di una replica del server upstream** casella di controllo.
...
7.  Se si desidera connettersi al server proxy usando le credenziali utente specifico, selezionare la **usare le credenziali dell'utente per connettersi al server proxy** casella di controllo e quindi digitare il nome utente, dominio e password dell'utente nel corrispondente finestre. Se si desidera abilitare l'autenticazione di base per l'utente che effettua la connessione al server proxy, selezionare la **Consenti autenticazione di base (password inviata in testo non crittografato)** casella di controllo.

8.  Fare clic su **Avanti**. Nel **Connetti al Server Upstream** pagina, fare clic su **avviare i Connecting**.

9. Effettuata la connessione, scegliere **Avanti** per continuare.

10. Nel **scelta lingue** pagina, è possibile selezionare le lingue da cui WSUS riceverà gli aggiornamenti - tutte le lingue o un sottoinsieme di lingue. selezione di un sottoinsieme di lingue salverà lo spazio su disco, ma è importante scegliere tutte le lingue necessarie per tutti i client di questo server WSUS. Se si sceglie di scaricare aggiornamenti solo per determinate lingue, selezionare **Scarica aggiornamenti solo nelle lingue seguenti**e quindi selezionare le lingue per cui si desidera che gli aggiornamenti; in caso contrario, lascia selezionata l'impostazione predefinita.

    > [!WARNING]
    > Se si seleziona l'opzione **Scarica aggiornamenti solo nelle lingue seguenti**e server è connesso un server WSUS downstream, questa opzione forzerà il server downstream è anche possibile utilizzare solo le lingue selezionate.

11. Dopo aver selezionato le opzioni linguistiche appropriate per la distribuzione, fare clic su **Avanti** per continuare.

12. Nella pagina **Scelta prodotti** è possibile specificare i prodotti per cui si desiderano gli aggiornamenti. Selezionare le categorie di prodotto, ad esempio Windows o prodotti specifici, ad esempio Windows Server 2008. Se si seleziona una categoria di prodotto consente di selezionare tutti i prodotti in tale categoria.

13. Selezionare le opzioni di prodotto appropriate per la distribuzione e quindi fare clic su **successivo**.
...
15. La pagina **Imposta pianificazione della sincronizzazione** consente di scegliere se eseguire la sincronizzazione in modo manuale o automatico.
    ...
    -   Se si sceglie **sincronizzare manualmente**, è necessario avviare il processo di sincronizzazione dalla Console di amministrazione di WSUS.

    -   Se si sceglie **sincronizzazione automatica**, il server WSUS verrà sincronizzato a intervalli prestabiliti.
...
17. Nella pagina **Operazione completata** è ora possibile avviare la sincronizzazione selezionando la casella di controllo **Avvia sincronizzazione iniziale**. Se non si seleziona questa opzione, è necessario usare la Console di gestione WSUS per eseguire la sincronizzazione iniziale. Per informazioni sulle impostazioni aggiuntive, fare clic su **Avanti** ; altrimenti, scegliere **Fine** per terminare la procedura guidata e completare la configurazione iniziale di Windows Server Update Services.
...
1.  Nella Console di amministrazione di WSUS, sotto **Update Services**, espandere il server WSUS, espandere **computer**, fare doppio clic su **tutti i computer**, quindi fare clic su **Aggiungi gruppo di computer**.

2.  Nel **Aggiungi gruppo di computer** nella finestra di dialogo **Name**, specificare il nome del nuovo gruppo e quindi fare clic su **aggiungere**.

3.  Fare clic su **computer**e quindi selezionare i computer che si desidera assegnare al nuovo gruppo.

4.  Fare doppio clic sui nomi dei computer selezionati nel passaggio precedente e quindi fare clic su **modificare l'appartenenza**.

5.  Nel **imposta l'appartenenza al gruppo di computer** finestra di dialogo, selezionare il gruppo test creato, quindi fare clic su **OK**.

## <a name="BKM_ConfigureClientUpdates"></a>2.4. Configurare gli aggiornamenti client
...
-   In un ambiente che Usa servizio directory active directory, è possibile utilizzare un esistente basato su dominio oggetto (criteri di gruppo) o creare un nuovo oggetto Criteri di gruppo.

-   In un ambiente senza active directory, usare l'editor Criteri di gruppo locali per configurare gli aggiornamenti automatici e quindi puntare i computer client al server WSUS.

> [!IMPORTANT]
> Le procedure seguenti presuppongono che la rete esegue active directory. Presuppongono inoltre che si abbia familiarità con Criteri di gruppo e lo si utilizzi per gestire la rete.
...
-   [2.3. Configurare gruppi di computer](2-configure-wsus.md#BKMK_ConfigcomputerGroups) in questo argomento

### <a name="BKMK_ConfigureAU"></a>Configurare gli aggiornamenti automatici in Criteri di gruppo
Se è stato configurato active directory nella rete, è possibile configurare uno o più computer contemporaneamente includerli in un oggetto Criteri di gruppo (GPO) e quindi configurando quell'oggetto con le impostazioni WSUS. Si consiglia di creare un nuovo oggetto Criteri di gruppo che contenga solo le impostazioni WSUS.

Collegare questo oggetto Criteri di gruppo WSUS a un contenitore di active directory appropriato per l'ambiente. In un ambiente semplice è possibile collegare un solo oggetto Criteri di gruppo WSUS al dominio. In un ambiente più complesso, è possibile collegare più oggetti Criteri di gruppo WSUS a diverse unità organizzative, in modo da applicare impostazioni di criteri WSUS diverse a tipi di computer differenti.

##### <a name="to-enable-wsus-through-a-domain-gpo"></a>Per abilitare WSUS tramite un oggetto Criteri di gruppo basato su dominio

1.  Nei criteri di gruppo Management Console (GPMC), passare a oggetto Criteri di gruppo in cui si desidera configurare WSUS e quindi fare clic su **modifica**.

2.  Nella console GPMC, espandere **configurazione computer**, espandere **criteri**, espandere **modelli amministrativi**, espandere **i componenti di Windows**, quindi fare clic su **Windows Update**.
...
5.  Selezionare **abilitare la destinazione lato client**, selezionare **Enabled**e quindi digitare il nome del gruppo di computer WSUS a cui si desidera aggiungere il computer nella **nome del gruppo di destinazione per questo computer**  casella.

    > [!NOTE]
    > L'opzione **Abilita appartenenza gruppo destinazione** consente ai computer client di aggiungersi ai gruppi di computer di destinazione nel server WSUS quando viene eseguito il reindirizzamento di Aggiornamenti automatici a un server WSUS. Se lo stato è impostato su abilitato, il computer verrà identificarsi come membro di un determinato gruppo di computer quando invia informazioni al server WSUS, che lo utilizza per determinare quali aggiornamenti vengono distribuiti a questo computer. Questa impostazione indica al server WSUS il gruppo che il computer client userà. È necessario creare il gruppo nel server WSUS e aggiungere al gruppo i computer membri del dominio.
...
9. Fare clic su **Abilitato** e quindi nelle caselle di testo **Impostare il servizio di aggiornamento nella rete Intranet per il rilevamento degli aggiornamenti** e **Impostare il server per le statistiche nella rete Intranet** digitare lo stesso URL del server WSUS. Ad esempio, digitare *http://servername* in entrambe le caselle (dove *servername* è il nome del server WSUS).

    > [!WARNING]
    > Durante l'immissione dell'indirizzo Intranet del server WSUS accertarsi di specificare la porta da utilizzare. Per impostazione predefinita, WSUS utilizza le porte 8530 per HTTP e 8531 per HTTPS. Ad esempio, se si Usa HTTP, è necessario digitare **http://servername:8530**.

10. Fare clic su **OK**.

Dopo aver configurato un computer client, richiederà alcuni minuti prima che il computer sia incluso nel **computer** pagina nella Console di amministrazione di WSUS. Se i computer client sono configurati con un oggetto Criteri di gruppo basato su dominio, per applicare le nuove impostazioni dei criteri al computer client tramite Criteri di gruppo saranno necessari circa 20 minuti. Per impostazione predefinita, gli aggiornamenti di criteri di gruppo in background ogni 90 minuti, con un offset casuale compreso tra 0 a 30 minuti. Se si desidera aggiornare prima i criteri di gruppo, è possibile aprire un prompt dei comandi nel client computer e digitare gpupdate /force.

per i computer client sono configurati utilizzando l'editor Criteri di gruppo locali, l'oggetto Criteri di gruppo viene applicata immediatamente e l'aggiornamento richiede circa 20 minuti. Se si inizia il rilevamento manuale, non devi attendere 20 minuti per il computer client contattare WSUS.
...
2.  Digitare wuauclt.exe. exe /detectnow e quindi premere INVIO.

## <a name="bkmk_2.5.ConfigSSL"></a>2.5. Proteggere WSUS con il protocollo Secure Sockets Layer
...
### <a name="ssllmits"></a>Limitazioni delle distribuzioni di WSUS SSL
...
2.  Se si usa WSUS con un database di SQL Server remoto, la connessione tra il server WSUS e il server di database non è protetta da SSL. Se è necessario proteggere la connessione al database, tenere presente quanto segue:
...
### <a name="consswsus"></a>Configurare SSL nel server WSUS
...
-   È possibile usare qualsiasi porta per SSL. Tuttavia, la porta configurata per SSL determina anche la porta usata da WSUS per inviare il traffico HTTP non crittografato. Considera gli esempi che seguono:
    ...
    -   Se si usa qualsiasi porta diversa dalla 443 per il traffico HTTPS, WSUS invia il traffico HTTP non crittografato attraverso la porta che precede numericamente la porta per HTTPS. Ad esempio, se si usa la porta 8531 per HTTPS, WSUS userà la porta 8530 per HTTP.
...
2.  Passare a **avviare**, tipo **CMD**, fare doppio clic su **prompt dei comandi**, quindi fare clic su **Esegui come amministratore**.
...
### <a name="consslclient"></a>Configurare SSL nei computer client
Quando si configura SSL nei computer client, è necessario considerare le problematiche seguenti:

-   È necessario includere un URL per una porta sicura nel server WSUS. Poiché non è possibile richiedere SSL nel server, l'unico modo per assicurarsi che i computer client possano usare un canale di sicurezza consiste nell'usare un URL che specifichi HTTPS. Se si usa qualsiasi porta diversa dalla 443 per SSL, è necessario includere anche tale porta nell'URL.

-   Il certificato in un computer client deve essere importato nell'archivio Autorità di certificazione radice attendibili del computer locale o archivio CA radice attendibile del servizio Aggiornamenti automatici. Se il certificato viene importato solo lo store autorità di certificazione radice attendibili dell'utente locale, gli aggiornamenti automatici avrà esito negativo l'autenticazione server.
...
### <a name="consslds"></a>Configurare SSL per il server WSUS downstream
...
2.  Fare clic su **avviare**, fare clic su **tutti i programmi**, fare clic su **strumenti di amministrazione**, quindi fare clic su **Windows Server Update Services**.
...
6.  digitare il nome del server upstream nella **nome Server** casella di testo. digitare il numero di porta utilizzato dal server per le connessioni SSL nel **numero porta** casella di testo.

7.  Selezionare il **Usa SSL per la sincronizzazione delle informazioni di aggiornamento** casella di controllo e quindi fare clic su **OK**.

### <a name="addlsl"></a>risorse aggiuntive su SSL
...
-   [Guida dettagliata di infrastruttura a chiave pubblica Suite B](https://go.microsoft.com/fwlink/?LinkID=203858)

-   [Implementazione e Gestione modelli di certificato](https://go.microsoft.com/fwlink/?LinkID=203859)

-   [Guida alla migrazione e Active directory certificato di servizi di aggiornamento](https://go.microsoft.com/fwlink/?LinkID=203860)
...
Per impostazione predefinita, l'accesso in lettura anonimo è abilitato in tutti i siti Web IIS nuovi e in quello predefinito. Alcune applicazioni, in particolare Windows SharePoint Services, possono rimuovere l'accesso anonimo. In questo caso, è necessario abilitare nuovamente l'accesso in lettura anonimo prima che si possono installare e usare WSUS correttamente.
...
3.  **Revoca/aggiornamenti del certificato**. Se si vuole aggiornare o revocare un certificato (ad esempio dopo l'individuazione di scadenza), WSUS non offre funzionalità per abilitare questa opzione. A tale scopo, è necessaria un'attività manuale molto complessa da svolgere manualmente o automatizzare correttamente.
