...
4. L'output sarà simile al seguente:
...
1. Usare l'ID \( di processo PID \) per arrestare il processo del daemon. Per trovare il PID, esaminare la seconda colonna dell'output o usare `pidof` . I daemon Hyper-V vengono eseguiti come root, quindi sono necessarie le autorizzazioni radice.

    ``` BASH
    sudo kill -15 `pidof hv_kvp_daemon`
    ```

1. Per verificare che tutti i `hv_kvp_daemon` processi siano finiti, eseguire:
...
1. Per verificare che il `hv_kvp_daemon` processo sia elencato con un nuovo ID processo, eseguire:
...
| R2 per Windows Server 2012 | Windows Update | |
...
| Windows Small Business Server 2011 | Windows Update | Non sotto il supporto mainstream ([altre](https://support.microsoft.com/lifecycle?p1=15817)informazioni). |
| - | | |
...
| Windows Server 2016 | Windows Update | |
| Windows Server, Canale semestrale | Windows Update | |
| R2 per Windows Server 2012 | Disco dei servizi di integrazione | Vedere [le istruzioni](#install-or-update-integration-services)riportate di seguito. |
