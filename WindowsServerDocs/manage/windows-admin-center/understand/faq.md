...
Sì. Windows Admin Center è disponibile a livello globale e pronto per ampie distribuzioni di produzione e utilizzo. In quanto piattaforma che continuerà a crescere e consolidarsi, chiudere l'anteprima significa che le funzionalità e gli strumenti principali della piattaforma corrente soddisfano i criteri di versione standard di Microsoft e il livello di qualità per usabilità, affidabilità, prestazioni, accessibilità, sicurezza e adozione.
...
* Per gli amministratori IT che non sono in grado di aggiornare frequentemente o che desiderano più tempo di convalida per le versioni che utilizzano nell'ambiente di produzione, questa è la versione adatta. La release (GA) disponibile a livello generale corrente è Windows Admin Center 1809.5.
* Per ottenere la versione GA più recente, [effettuare il download qui](https://aka.ms/WACDownload).

* Versioni di Windows Admin Center a livello generale sono supportate in modo continuo, basato su Microsoft [criteri del ciclo di vita moderni](https://support.microsoft.com/help/30881/modern-lifecycle-policy). Ciò significa che solo l'ultima versione di disponibilità generale di Windows Admin Center è gestita e supportata, mentre gli utenti devono "al passo coi" eseguendo l'aggiornamento all'ultima versione disponibile a livello generale di Windows Admin Center entro 30 giorni di disponibilità deve rimanere supportato. Questo criterio si applica a sia la piattaforma Windows Admin Center, nonché tutte le estensioni Microsoft sono a livello generale e pubblicati nell'estensione Windows Admin Center feed. Si noti che alcune estensioni potrebbero essere aggiornati più frequentemente rispetto ad altri, tra le versioni di Windows Admin Center a livello generale.
