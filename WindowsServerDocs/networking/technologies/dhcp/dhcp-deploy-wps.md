...
Si consiglia, ad esempio, di configurare manualmente l'indirizzo IP del server DHCP e gli indirizzi IP dei server DNS e dei controller di dominio che eseguono Active Directory Domain Services \(servizi di dominio Active Directory.\)
...
I requisiti per il Lab variano a seconda del fatto che si utilizzino macchine \(\)virtuali o server fisici, nonché se si utilizza un dominio Active Directory o si distribuisce un server DHCP autonomo.
...
Per creare gruppi di sicurezza, è necessario eseguire un comando \(Netsh\) della shell di rete in Windows PowerShell, quindi riavviare il servizio DHCP in modo che i nuovi gruppi diventino attivi.
...
### <a name="notify-server-manager-that-post-install-dhcp-configuration-is-complete-optional"></a>Notifica Server Manager che la\-configurazione DHCP post-installazione \(è stata completata facoltativamente\)

Dopo aver completato le attività\-post-installazione, ad esempio la creazione di gruppi di sicurezza e l'autorizzazione del server DHCP in Active Directory, Server Manager possibile che venga visualizzato un avviso nell'interfaccia utente\- che informa il post per completare la procedura di installazione, è necessario utilizzare la configurazione guidata post-installazione DHCP.

È possibile impedire\-che venga visualizzato un messaggio non necessario e non accurato in Server Manager configurando la seguente chiave del registro di sistema utilizzando questo comando di Windows PowerShell.
...
### <a name="set-server-level-dns-dynamic-update-configuration-settings-optional"></a>Impostare le impostazioni \(di configurazione dell'aggiornamento dinamico DNS a livello di server facoltativo\)
