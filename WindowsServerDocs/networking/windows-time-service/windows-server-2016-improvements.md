...
I primi 3 contatori destinati a scenari per la risoluzione dei problemi di accuratezza.  La risoluzione dei problemi di precisione di tempo e NTP sezione in seguito, [consigliate](#BestPractices), con ulteriori dettagli.
...
I membri del dominio utilizzano la gerarchia dei domini per determinare quali computer utilizzano come origine per sincronizzare l'ora.  Ogni membro del dominio troverà un altro computer per la sincronizzazione con e salvarlo come dell'orologio di origine.  Ogni tipo di membro del dominio segue un diverso set di regole per un orologio di origine di sincronizzazione dell'ora.  Il PDC nella radice della foresta è l'origine di clock predefinita per tutti i domini.  Di seguito sono elencati diversi ruoli e una descrizione di livello elevato per essi ricerca per un'origine:
...
Gli algoritmi di protocollo NTP presentano una dipendenza da simmetria della rete.  L'aumento del numero di hop di rete, la probabilità di asimmetria cresce.  Non esiste, è difficile prevedere quali tipi di accuratezza verrà visualizzato in ambienti specifici. 
