---
title: "Parità accelerata con mirroring"
...
---
# <a name="mirror-accelerated-parity"></a>Parità accelerata con mirroring
...
- Se si usa la parità, invece, è necessario ricalcolarla per ogni operazione di scrittura, causando prestazioni di scrittura casuali. La parità tuttavia consente agli utenti di archiviare i dati con una maggiore efficienza di spazio. (Per ulteriori informazioni sull'efficienza in termini spazio, leggi ![qui](../storage-spaces/Storage-Spaces-Fault-Tolerance.md)). 
...
1.  **Scritture su mirroring** 
     - **a.** Se la scrittura in ingresso modifica i dati esistenti nel mirroring, ReFS modificherà i dati sul posto. 
     - **b.** Se la scrittura in ingresso è una nuova scrittura e ReFS trova spazio libero sufficiente nel mirroring per gestire tale scrittura, ReFS scriverà nel mirroring. 

![Scrittura-su-mirror](media/mirror-accelerated-parity/Write-to-Mirror.png)

2. **Scritture su mirroring, riallocate da parità:**
    - **a.** Se la scrittura in ingresso modifica i dati che si trovano in parità e ReFS trova spazio libero sufficiente nel mirroring per gestire la scrittura in ingresso, ReFS invaliderà prima di tutto i dati precedenti in parità e quindi scriverà sul mirroring. Questo invalidamento rappresenta un'operazione sui metadati rapida e conveniente che consente di migliorare in modo significativo le prestazioni di scrittura nella parità.

![Scrittura-riallocata](media/mirror-accelerated-parity/Reallocated-Write.png)

3. **Scritture in parità:**
    - **a.** Se ReFS non trova spazio libero sufficiente nel mirroring, scriverà nuovi dati nella parità o modificherà direttamente i dati esistenti in parità. Nella sezione "Ottimizzazioni delle prestazioni" seguente vengono fornite indicazioni per ridurre il numero di scritture nella parità.

![Scritture-in-parità](media/mirror-accelerated-parity/Write-to-Parity.png)
...
- **Livello di mirroring > Scritture riallocate >> Livello parità**
...
>[!IMPORTANT]
>Per i carichi di lavoro Hyper-V con parità accelerata con mirroring, ReFS offre le migliori prestazioni quando i dischi rigidi virtuali ad alta percentuale di scrittura vengono distribuiti in più directory. Di conseguenza, per ottenere prestazioni ottimali, si consiglia di non inserire molti dischi rigidi virtuali ad alta percentuale di scrittura nella stessa sottodirectory.   
...
### <a name="increasing-the-size-of-the-mirrored-tier"></a>Aumento delle dimensioni del volume di mirroring: 
