...
## <a name="how-does-dedup-work"></a>Come funziona la deduplicazione dati?

La Deduplicazione dati in WindowsServer è stata creata con i due principi seguenti:

1. **L'ottimizzazione non deve ostacolare la scrittura su disco**  
...
|------------|-----------------|------------------|
| <a id="usage-type-default"></a>Valore predefinito | File server per uso generale:<ul><li>Condivisioni del team</li><li>Cartelle di lavoro</li><li>Reindirizzamento cartelle</li><li>Condivisioni per lo sviluppo di software</li></ul> | <ul><li>Ottimizzazione in background</li><li>Criteri predefiniti di ottimizzazione:<ul><li>Età minima del file = 3 giorni</li><li>Ottimizzazione dei file in uso = No</li><li>Ottimizzazione dei file parziali = No</li></ul></li></ul> |
...
| <a id="job-info-gc"></a>Garbage Collection | Il processo **Garbage Collection** richiede il recupero di spazio su disco rimuovendo blocchi inutili a cui i file che sono stati recentemente modificati o eliminati non fanno più riferimento. | Ogni sabato alle 2:35 |
...
| <a id="dedup-term-dedup"></a>Dedup | Un'abbreviazione di Deduplicazione dati usata in PowerShell, nelle API e nei componenti di WindowsServer e di uso comune nella community di WindowsServer. |
| <a id="dedup-term-file-metadata"></a>Metadati del file | Ogni file contiene metadati che descrivono le proprietà interessanti sul file che non sono correlate al contenuto principale del file. Ad esempio, data di creazione, data dell'ultima lettura, autore e così via. |
...
| <a id="dedup-term-workload"></a>Carico di lavoro | Un carico di lavoro è un'applicazione che viene eseguita su WindowsServer. Esempi dei carichi di lavoro includono file server a scopi generici, Hyper-V e SQL Server. |
...
**In che modo Deduplicazione dati è diversa da altri prodotti di ottimizzazione?**  
...
* *In che modo Deduplicazione dati è diversa dai formati di archivio come i file con estensione zip, rar, 7z, cab e così via?*  
...
**È possibile eseguire manualmente un processo di deduplicazione dati?**  
...
**È possibile modificare le pianificazioni predefinite per i processi di deduplicazione dati sul sistema?**  
