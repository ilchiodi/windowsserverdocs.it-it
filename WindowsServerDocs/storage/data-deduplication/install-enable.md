...
title: Installazione e abilitazione di Deduplicazione dati
...
In questo argomento viene descritto come installare [Deduplicazione dati](overview.md), valutare i carichi di lavoro per la deduplicazione e abilitare Deduplicazione dati su volumi specifici.
...
1. In Aggiunta guidata ruoli e funzionalità seleziona **Ruoli server** e quindi **Deduplicazione dati**.  
![Installare Deduplicazione dati tramite Server Manager: selezionare Deduplicazione dati da Ruoli server](media/install-dedup-via-server-manager-1.png)
...
## <a id="enable-dedup"></a>Abilitare Deduplicazione dati
...
* *I carichi di lavoro consigliati* che hanno dimostrato di avere sia set di dati per cui la deduplicazione si è rivelata molto vantaggiosa sia modelli di uso delle risorse compatibili con il modello di post-elaborazione di Deduplicazione dati. È consigliabile [abilitare sempre Deduplicazione dati](install-enable.md#enable-dedup-lights-on) su questi carichi di lavoro:
...
### <a id="enable-dedup-evaluating-sometimes-workloads"></a>Valutare i carichi di lavoro per Deduplicazione dati
> [!Important]  
...
2. **Come sono i modelli IO del carico di lavoro rispetto al set di dati relativo? Quali prestazioni ha il carico di lavoro?**  
...
3. **Quali sono i requisiti di risorse del carico di lavoro sul server?**  
...
### <a id="enable-dedup-lights-on"></a>Abilitare Deduplicazione dati
Prima di abilitare Deduplicazione dati, è necessario scegliere il [tipo di utilizzo](understand.md#usage-type) che rispecchia maggiormente il carico di lavoro. I tipi di utilizzo inclusi in Deduplicazione dati sono tre.
...
#### <a id="enable-dedup-via-server-manager"></a>Abilitare Deduplicazione dati tramite Server Manager
...
#### <a id="enable-dedup-via-powershell"></a>Abilitare Deduplicazione dati tramite PowerShell
...
In WindowsServer 2012 e WindowsServer 2012 R2, i volumi dovevano essere ridimensionati con attenzione per assicurarsi che Deduplicazione dati potesse far fronte alla varianza nel volume. Questo significava in genere che la dimensione massima media di un volume deduplicato per un carico di lavoro di varianza elevato era 1-2 TB e la dimensione assoluta massima consigliata era 10 TB. In WindowsServer 2016 queste limitazioni sono state rimosse. Per altre informazioni, vedere [What's new in Data Deduplication](whats-new.md#large-volume-support) (Novità di Deduplicazione dati).
...
**Quali sono i requisiti di memoria per Deduplicazione dati?**  
...
**Quali sono i requisiti di archiviazione per Deduplicazione dati?**  
In WindowsServer 2016 Deduplicazione dati supporta dimensioni di volume fino a 64 TB. Per altre informazioni, vedere [What's new in Data Deduplication](whats-new.md#large-volume-support) (Novità di Deduplicazione dati).
