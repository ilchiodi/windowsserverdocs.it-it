---
title: Passaggio 2 piano avanzato delle distribuzioni di DirectAccess
...
ms.locfileid: "59845302"
---
# <a name="step-2-plan-advanced-directaccess-deployments"></a>Passaggio 2 piano avanzato delle distribuzioni di DirectAccess
...
|----|--------|  
|[2.1 pianificare la distribuzione del client](#bkmk_21client)|Pianificare la modalità di connessione dei computer client con DirectAccess. Individuare i computer gestiti da configurare come client DirectAccess e pianificare la distribuzione di Assistente connettività di rete o di DirectAccess Connectivity Assistant nei computer client.|  
|[2.2 pianificare la distribuzione del server DirectAccess](#bkmk_22server)|Pianificare la modalità di distribuzione del server DirectAccess.|  
|[2.3 pianificare i server di infrastruttura](#bkmk_23Infservers)|Pianificare il server dell'infrastruttura per la distribuzione DirectAccess, incluso il server dei percorsi di rete DirectAccess, i server DNS (Domain Name System) e i server di gestione DirectAccess.|  
|[2.4 pianificare i server applicazioni](#bkmk_AppServers)|Pianificare i server applicazioni IPv4 e IPv6 e, facoltativamente, valutare se è necessaria l'autenticazione end-to-end tra i computer client DirectAccess e i server applicazioni interni.|  
|[2.5 pianificare DirectAccess e i client VPN di terze parti](#bkmk_DAandVPN)|Quando si distribuisce DirectAccess con client VPN di terze parti, può essere necessario impostare un valore del Registro di sistema per consentire la coesistenza delle due soluzioni di accesso remoto.|  
  
## <a name="bkmk_21client"></a>2.1 pianificare la distribuzione del client  
...
3.  Quali impostazioni vengono configurate per l'Assistente connettività di rete?  
    ...
    -   **Un indirizzo di posta elettronica dell'Help Desk**  
...
## <a name="bkmk_22server"></a>2.2 pianificare la distribuzione del server DirectAccess  
...
-   **Topologia di rete**  
    ...
    Per altre informazioni sulla selezione della topologia per la distribuzione, vedere [1.1 topologia di rete di piano e le impostazioni](da-adv-plan-s1-infrastructure.md#bkmk_11Networksvrtopsettings).  
...
-   **Prefissi IPv6**  
    ...
    Se la Configurazione guidata del server di Accesso remoto rileva che IPv6 è stato distribuito sulle schede di rete, vengono automaticamente popolati i prefissi IPv6 per la rete interna, un prefisso IPv6 da assegnare ai computer client DirectAccess e un prefisso IPv6 da assegnare ai computer client VPN. Se i prefissi generati automaticamente non sono corretti per l'infrastruttura IPv6 nativa, è necessario modificarli manualmente. Per altre informazioni, vedere [1.1 topologia di rete di piano e le impostazioni](da-adv-plan-s1-infrastructure.md#bkmk_11Networksvrtopsettings).  
  
-   **Autenticazione**  
  
    Decidere la modalità di autenticazione dei client DirectAccess nel server DirectAccess:  
  
    -   **Autenticazione utente**. È possibile abilitare l'autenticazione utente con le credenziali Active Directory o con un'autenticazione a due fattori. Per altre informazioni sull'autenticazione con l'autenticazione a due fattori, vedere [distribuire accesso remoto con autenticazione OTP](https://technet.microsoft.com/library/hh831379.aspx).  
  
    -   **Autenticazione computer**. È possibile configurare l'autenticazione computer in modo che usi i certificati o il server DirectAccess come proxy Kerberos per conto del client. Per altre informazioni, vedere [1.3 requisiti dei certificati piano](da-adv-plan-s1-infrastructure.md#bkmk_12CAsandCerts).  
  
    -   **I client Windows 7**. Per impostazione predefinita, i computer client che eseguono Windows 7 non è possibile connettersi a una distribuzione di Windows Server 2012 R2 o Windows Server 2012 DirectAccess. Se si dispone di client che eseguono Windows 7 nell'organizzazione e richiedono l'accesso remoto alle risorse interne, è possibile consentire loro di connettersi. Qualsiasi computer client al quale si intende concedere l'accesso alle risorse interne deve essere membro di un gruppo di sicurezza specificato nella Configurazione guidata client DirectAccess.  
  
        > [!NOTE]  
        > Per consentire i client che eseguono Windows 7 alla connessione tramite DirectAccess, è necessario usare l'autenticazione del certificato computer.  
...
## <a name="bkmk_23Infservers"></a>2.3 pianificare i server di infrastruttura  
DirectAccess richiede tre tipi di server dell'infrastruttura:  
  
-   **Server DNS**. Per altre informazioni, vedere [1.4 Pianificare i requisiti DNS](da-adv-plan-s1-infrastructure.md#bkmk_14Dns)  
  
-   **Server dei percorsi di rete**. Per altre informazioni, vedere [1.5 Pianificare il server dei percorsi di rete](da-adv-plan-s1-infrastructure.md#bkmk_14NLS)  
  
-   **Server di gestione**. Per altre informazioni, vedere [1.6 Pianificare i server di gestione](da-adv-plan-s1-infrastructure.md#bkmk_15mgmtservers)  
  
## <a name="bkmk_AppServers"></a>2.4 pianificare i server applicazioni  
...
Per impostazione predefinita, quando si estende l'autenticazione ai server applicazioni, il payload dati viene crittografato tra il client DirectAccess e il server applicazioni. Si può scegliere di non crittografare il traffico e usare solo l'autenticazione. Tuttavia, è però meno sicuro rispetto all'uso di autenticazione e crittografia ed è supportata solo per server applicazioni che eseguono Windows Server 2008 R2, i sistemi operativi Windows Server 2012.  
  
## <a name="bkmk_DAandVPN"></a>2.5 pianificare DirectAccess e i client VPN di terze parti  
