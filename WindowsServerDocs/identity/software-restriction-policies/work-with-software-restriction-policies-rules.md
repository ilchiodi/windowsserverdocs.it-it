...
Questo argomento descrive le procedure di utilizzo di certificati, percorso, internet delle regole di zona e hash usando criteri di restrizione Software.

## <a name="introduction"></a>Introduzione
I criteri di restrizione software, è possibile proteggere l'ambiente informatico da software non attendibile identificando e specificando il software può essere eseguita. È possibile definire un livello di sicurezza predefinito **Unrestricted** oppure **vietate** per un oggetto Criteri di gruppo (GPO) in modo che software può essere concessa o non è consentito eseguire per impostazione predefinita. È possibile creare eccezioni per questo livello di sicurezza predefinito mediante la creazione di regole di criteri software specifico di restrizione software. Se il livello di sicurezza predefinito è impostato su **Non consentito**, ad esempio, è possibile creare regole per consentire l'esecuzione di software specifico. I tipi di regole sono come segue:
...
> -   Criteri di restrizione software rendere effettive, gli utenti devono aggiornare le impostazioni dei criteri disconnettendosi dal e accedere al proprio computer.
> -   Quando più di una regola di criteri di restrizione software viene applicata alle impostazioni dei criteri, si verifica una precedenza delle regole per la gestione dei conflitti.
...
-   [Per un oggetto Criteri di gruppo e si trovano in un server appartenente a un dominio](#BKMK_2)

-   [Per un oggetto Criteri di gruppo e si sono in un controller di dominio o nella workstation con strumenti di amministrazione Server remota installato](#BKMK_3)

-   [Per dominio solo controller e si sia in un controller di dominio o in una workstation in cui il pacchetto di strumenti di amministrazione Server remota installato](#BKMK_4)

#### <a name="BKMK_1"></a>Per abilitare le regole certificati per il computer locale

1.  Aprire Impostazioni sicurezza locale.

2.  Nell'albero della console, fare clic su **opzioni di sicurezza** in sicurezza impostazioni/criteri locali.

3.  Nel riquadro dei dettagli fare doppio clic **le impostazioni di sistema: Utilizzare le regole certificati nel file eseguibili di Windows per i criteri di restrizione Software**.
...
#### <a name="BKMK_2"></a>Per abilitare le regole certificati per un oggetto Criteri di gruppo e si trovano in un server appartenente a un dominio
...
7.  Nell'albero della console, fare clic su **opzioni di sicurezza** che si trova sotto *Oggettocriteridigruppo* [*ComputerName*] Computer/criteri di configurazione/Windows le impostazioni o della sicurezza Impostazioni/criteri locali /.

8.  Nel riquadro dei dettagli fare doppio clic **le impostazioni di sistema: Utilizzare le regole certificati nel file eseguibili di Windows per i criteri di restrizione Software**.
...
#### <a name="BKMK_3"></a>Per abilitare il certificato regole per un oggetto Criteri di gruppo e si usa un controller di dominio o in una workstation con strumenti di amministrazione Server remota installato
...
5.  Nell'albero della console, fare clic su **opzioni di sicurezza** che si trova sotto *Oggettocriteridigruppo*[*ComputerName*] Computer/criteri di configurazione/Windows le impostazioni o della sicurezza Impostazioni/criteri locali.

6.  Nel riquadro dei dettagli fare doppio clic **le impostazioni di sistema: Utilizzare le regole certificati nel file eseguibili di Windows per i criteri di restrizione Software**.
...
#### <a name="BKMK_4"></a>Per abilitare le regole certificati solo per i controller di dominio e si sono in un controller di dominio o in una workstation con strumenti di amministrazione Server remota installato
...
### <a name="set-trusted-publisher-options"></a>Impostare le opzioni di pubblicazione trusted
...
1.  Nel **avviare** digitare**gpedit. msc** e quindi premere INVIO.
...
2.  Nell'albero della console, fare doppio clic su **oggetti Criteri di gruppo** nella foresta e nel dominio che contiene il **criterio dominio predefinito** oggetto Criteri di gruppo (GPO) che si desidera modificare.
...
1.  Nel **avviare** dello schermo, un tipo **gpedit. msc** nel **Cerca programmi e file** o in Windows 8, su Desktop e quindi premere INVIO.
...
2.  Nell'albero della console, fare doppio clic su **oggetti Criteri di gruppo** nella foresta e nel dominio che contiene il **criterio dominio predefinito** oggetto Criteri di gruppo che si desidera modificare.
...
2.  Nell'albero della console o il riquadro dei dettagli, fare clic sulla **regole aggiuntive**, quindi fare clic su **nuova regola Hash**.

3.  Fare clic su **esplorare** per trovare un file.

    > [!NOTE]
    > In Windows XP è possibile incollare un hash in pre-calcolato **hash File**. In Windows Server 2008 R2, Windows 7 e versioni successive, questa opzione non è disponibile.
...
> -   Se si desidera che altri utenti di utilizzare una regola hash in modo che non è possibile eseguire un virus, calcolare l'hash del virus tramite criteri di restrizione software e quindi inviare tramite posta elettronica il valore hash per gli altri utenti. Mai tramite posta elettronica il virus stesso.
...
> -   Un file rinominato o spostato in un'altra cartella mantiene lo stesso hash. Qualsiasi modifica al file stesso comporta un hash diverso.
...
2.  Nell'albero della console o il riquadro dei dettagli, fare clic sulla **regole aggiuntive**, quindi fare clic su **nuova regola area Internet**.

3.  In **Area Internet** fare clic su un'area Internet.

4.  In **a livello di sicurezza**, fare clic su **non consentito** oppure **senza restrizioni**, quindi fare clic su **OK**.
...
2.  Nell'albero della console o il riquadro dei dettagli, fare clic sulla **regole aggiuntive**, quindi fare clic su **nuova regola di percorso**.
...
> [!CAUTION]
> -   In determinate cartelle, ad esempio la cartella di Windows, impostare la sicurezza a livello di **vietate** può influire negativamente sul funzionamento del sistema operativo. Assicurarsi di non bloccare un componente cruciale del sistema operativo o uno dei programmi dipendenti.
...
> -   Per impedire agli utenti di eseguire gli allegati di posta elettronica, è possibile creare una regola di percorso per la directory degli allegati del programma di posta elettronica che impedisce agli utenti di eseguire gli allegati di posta elettronica.
...
1.  Nel **avviare** digitare regedit.
...
4.  Nell'albero della console o il riquadro dei dettagli, fare clic sulla **regole aggiuntive**, quindi fare clic su **nuova regola di percorso**.

5.  Nelle **percorso**, incollare il nome della chiave del Registro di sistema, seguito dal nome del valore.

6.  Segni di percentuale, %, ad esempio, % HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\PlatformSDK\Directories\InstallDir%, racchiudere il percorso del Registro di sistema.

7.  Nelle **a livello di sicurezza**, fare clic su **non consentito** oppure **Unrestricted**.
