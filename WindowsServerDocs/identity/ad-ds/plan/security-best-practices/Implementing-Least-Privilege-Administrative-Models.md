...
Le istruzioni dettagliate per l'implementazione di questi controlli sono disponibili in [Appendix H: Protezione degli account di amministratore locale e dei gruppi @ no__t-0. Prima di implementare queste impostazioni, tuttavia, assicurarsi che gli account amministratore locali non in uso nell'ambiente per eseguire i servizi in computer o eseguire altre attività per cui questi account non devono essere utilizzati. Queste impostazioni di test completi prima di implementarli in un ambiente di produzione.  
...
*Law numero sei: Un computer è sicuro solo come amministratore attendibile.* - [dieci leggi di sicurezza non modificabili (versione 2,0)](https://technet.microsoft.com/security/hh278941.aspx)  

Le informazioni fornite in questo caso sono destinate a fornire linee guida generali per proteggere gli account con privilegi più elevato e i gruppi in Active Directory. Sono inoltre disponibili istruzioni dettagliate in [Appendix D: Protezione degli account amministratore predefiniti in Active Directory @ no__t-0, [Appendix E: Protezione dei gruppi Enterprise Admins in Active Directory @ no__t-0, [Appendix F: Protezione dei gruppi Domain Admins in Active Directory @ no__t-0 e in [Appendix G: Protezione dei gruppi Administrators in Active Directory @ no__t-0.  
...
In ogni dominio della foresta, il criterio controller di dominio predefinito o un criterio collegato all'unità organizzativa controller di dominio deve essere modificato per aggiungere l'account amministratore di ciascun dominio ai seguenti diritti utente in **Impostazioni computer computer\Criteri\Impostazioni \Security protezione\Criteri locali\Assegnazione diritti assegnati**:  
...
Il gruppo Enterprise Admins, ospitato nel dominio radice della foresta, non deve contenere alcun utente su base giornaliera, con la possibile eccezione dell'account amministratore locale del dominio, purché sia protetto come descritto in precedenza e in [Appendix D: Protezione degli account amministratore predefiniti in Active Directory @ no__t-0.  

Quando è necessario accedere EA, gli utenti con account richiedono autorizzazioni e diritti EA devono essere temporaneamente inseriti nel gruppo Enterprise Admins. Anche se gli utenti utilizzino gli account con privilegi elevati, le relative attività devono essere controllate e preferibilmente eseguite con un utente che esegue le modifiche e un altro utente osservare le modifiche per ridurre al minimo la probabilità di uso improprio accidentale o errori di configurazione. Quando le attività sono state completate, gli account devono essere rimosso dal gruppo EA. Questo può essere ottenuto tramite procedure manuali e documentato processi, il software di gestione (PIM/PAM) di identità/accesso privilegiato terze parti o una combinazione di entrambi. Le linee guida per la creazione di account che possono essere usati per controllare l'appartenenza di gruppi con privilegi in Active Directory vengono fornite negli [account attraenti per il furto di credenziali](../../../ad-ds/plan/security-best-practices/Attractive-Accounts-for-Credential-Theft.md) e vengono fornite istruzioni dettagliate in [Appendix I: Creazione di account di gestione per account e gruppi protetti in Active Directory @ no__t-0.  
...
- Come descritto in precedenza, il gruppo Enterprise Admins non deve contenere alcun utente su base giornaliera, con la possibile eccezione dell'account amministratore del dominio radice della foresta, che deve essere protetto come descritto in [Appendix D: Protezione degli account amministratore predefiniti in Active Directory @ no__t-0.  
...
Come avviene con i gruppi di EA e DA, l'appartenenza al gruppo Administrators (BA) dovrebbe essere necessaria solo in scenari di compilazione o di ripristino di emergenza. Non devono essere presenti account utente giornalieri nel gruppo Administrators, ad eccezione dell'account amministratore locale per il dominio, se è stato protetto come descritto in [Appendix D: Protezione degli account amministratore predefiniti in Active Directory @ no__t-0.  
...
1. Bilancio Investendo lo sviluppo del controllo degli accessi in base al ruolo utilizzando software e strumenti di cui si è già proprietari, è possibile ridurre i costi software necessari per la distribuzione di una soluzione Tuttavia, a meno che non si dispone di personale di esperti nella creazione e distribuzione di soluzioni RBAC native, potrebbe essere necessario coinvolgere consulenze risorse per sviluppare la soluzione. È opportuno valutare attentamente i costi previsti per una soluzione personalizzata con i costi per distribuire una soluzione "out-of-box", in particolare se il budget è limitato.  
2. Composizione dell'ambiente IT: Se l'ambiente è costituito principalmente da sistemi Windows o se si sta già usando Active Directory per la gestione di account e sistemi non Windows, le soluzioni native personalizzate possono fornire la soluzione ottimale per le proprie esigenze. Se l'infrastruttura contiene molti sistemi che non sono in esecuzione Windows e non gestiti da Active Directory, è necessario prendere in considerazione le opzioni per la gestione dei sistemi non Windows separatamente dall'ambiente di Active Directory.  
3. Modello di privilegi nella soluzione: Se un prodotto si basa sul posizionamento dei propri account di servizio in gruppi con privilegi elevati in Active Directory e non offre opzioni che non richiedono privilegi eccessivi per il software RBAC, non è stato effettivamente ridotto il Active Directory attacco Surface è stata modificata solo la composizione dei gruppi con privilegi più elevati nella directory. A meno che un fornitore dell'applicazione può fornire controlli per gli account di servizio che riducono al minimo la probabilità che l'account viene compromesso e utilizzato da utenti malintenzionati, si consiglia di considerare altre opzioni.  
...
Se si implementano native RBAC e PIM, tuttavia, è consigliabile creare gli account che non dispongono di alcun privilegio e con la funzione di popolamento e depopulating con privilegi sola i gruppi in Active Directory quando necessario. [Appendice I: La creazione di account di gestione per gli account e i gruppi protetti in Active Directory @ no__t-0 fornisce istruzioni dettagliate che è possibile utilizzare per creare account a questo scopo.  

### <a name="implementing-robust-authentication-controls"></a>Implementazione di controlli di autenticazione affidabile

*Law numero sei: Ci siamo davvero qualcuno a provare a indovinare le password.* - [10 leggi immutabili di amministrazione della sicurezza](https://technet.microsoft.com/library/cc722488.aspx)  
