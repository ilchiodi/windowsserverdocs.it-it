...
|Database di configurazione di ADFS|Un database utilizzato per archiviare tutti i dati di configurazione che rappresentano una singola istanza di ADFS o servizio federativo. Questi dati di configurazione possono essere archiviati in un database di SQL Server o usando la funzionalità database interno di Windows incluso in Windows Server 2016, Windows Server 2012 e 2012 R2 e Windows Server 2008 e 2008 R2. </br></br>È possibile creare il database di configurazione di AD FS per SQL Server utilizzando lo strumento Fsconfig. exe\-line e per il database interno di Windows tramite la configurazione guidata del server federativo AD FS.|  
