...
> [!IMPORTANT]  
> L'aggiornamento funziona al meglio in macchine virtuali in cui non sono necessari specifici driver hardware OEM per l'aggiornamento.  
...
> [!IMPORTANT]  
> Se il server usa Gruppo NIC, disabilita questa funzionalità prima dell'aggiornamento e quindi riabilitala dopo che l'aggiornamento è stato completato. Per informazioni dettagliate, vedere [Panoramica di Gruppo NIC](https://technet.microsoft.com/library/hh831648(v=ws.11).aspx).

## Aggiornamento delle versioni definitive precedenti di Windows Server a Windows Server 2016
...
## Considerazioni relative all'aggiornamento per ruolo del server
...
## Conversione di una versione di valutazione corrente in una versione definitiva corrente

È possibile convertire la versione di valutazione di Windows Server 2016 Standard a Windows Server 2016 Standard (definitiva) o Datacenter (definitiva). Analogamente, puoi convertire la versione di valutazione di Windows Server 2016 Datacenter nella versione definitiva.

> [!IMPORTANT]  
> Per le versioni di Windows Server 2016 precedenti a 14393.0.161119-1705.RS1_REFRESH, è possibile eseguire solo la conversione dalla versione di valutazione a quella definitiva con Windows Server 2016 installato mediante l'opzione Esperienza desktop (non l'opzione Server Core). A partire dalla versione 14393.0.161119-1705. RS1_REFRESH, è possibile convertire le edizioni di valutazione in quella definitiva indipendentemente dall'opzione di installazione utilizzata.

Prima di tentare di eseguire la conversione dalla versione di valutazione a quella definitiva, verifica che nel server sia effettivamente in esecuzione una versione di valutazione. A tale scopo, eseguire una delle operazioni seguenti:
...
Per **Windows Server 2016 Essentials**: è possibile eseguire la conversione alla versione definitiva completa immettendo un codice Product Key per attivazione singola, per contratti multilicenza o OEM nel comando **slmgr.vbs**.
...
1.  Se il server è un **controller di dominio**, non è possibile eseguire la conversione a una versione definitiva. In questo caso, installare un controller di dominio aggiuntivo in un server che esegue una versione definitiva e rimuovere Active Directory Domain Services dal controller di dominio in esecuzione nella versione di valutazione. Per altre informazioni, vedere [Aggiornare controller di dominio a Windows Server 2012 R2 e Windows Server 2012](https://technet.microsoft.com/library/hh994618.aspx).
2.  Leggere le condizioni di licenza.
3.  Da un prompt dei comandi con privilegi elevati determinare il nome dell'edizione corrente tramite il comando **DISM /online /Get-CurrentEdition**. Prendere nota dell'ID dell'edizione, che corrisponde a una forma abbreviata del nome. Quindi eseguire **DISM /online /Set-Edition:\<ID edizione\> /ProductKey:XXXXX-XXXXX-XXXXX-XXXXX-XXXXX /AcceptEula**, specificando l'ID edizione e un codice product key per attivazione singola. Il server verrà riavviato due volte.
...
> Suggerimento Per altre informazioni su Dism.exe, vedi le [opzioni della riga di comando per Gestione e manutenzione immagini distribuzione](https://go.microsoft.com/fwlink/?LinkId=192466).

## Conversione da una versione definitiva corrente a un'altra versione definitiva corrente

In qualsiasi momento dopo l'installazione di Windows Server 2016 puoi eseguire il programma di installazione per ripristinare l'installazione (operazione chiamata a volte "ripristino sul posto") oppure, in determinati casi, per eseguire la conversione a un'edizione diversa.
...
Per Windows Server 2016 Standard, è possibile convertire il sistema in Windows Server 2016 Datacenter come segue: da un prompt dei comandi con privilegi elevati, determinare il nome dell'edizione corrente con il comando **DISM /online /Get-CurrentEdition**. Per Windows Server 2016 Standard sarà `ServerStandard`. Esegui il comando **DISM /online /Get-TargetEditions** per ottenere l'ID dell'edizione è possibile eseguire l'aggiornamento. Prendere nota di questo ID edizione, una forma abbreviata del nome. Quindi Esegui **DISM /online /Set-Edition: \ < \<ID > /ProductKey: XXXXX-XXXXX-XXXXX-XXXXX /AcceptEula**, che fornisce l'ID dell'edizione di destinazione della e il codice product key. Il server verrà riavviato due volte.

## Conversione da una versione con contratto multilicenza corrente a una versione definitiva corrente

In qualsiasi momento dopo l'installazione di Windows Server 2016 puoi eseguire la conversione tra una versione definitiva, una versione con contratto multilicenza o una versione OEM. L'edizione resta la stessa durante la conversione. Se inizi con una versione di valutazione, convertila prima di tutto in versione definitiva e quindi potrai convertirla di nuovo nei modi descritti qui.

A questo scopo, da un prompt dei comandi con privilegi elevati esegui: **slmgr /ipk \<key\>**
