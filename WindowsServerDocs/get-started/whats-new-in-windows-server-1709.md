...
- **Canale semestrale**: si tratta di un vantaggio di Software Assurance completamente supportato in ambienti di produzione. La differenza è che è supportato per 18 mesi e sarà disponibile una nuova versione ogni sei mesi.
...
| Convenzione di denominazione  | Windows Server, versione AAMM  | WindowsServer AAAA |
...
## Contenitori applicazioni e micro servizi
...
**Contenitore nano e contenitore di Server Core**: prima di tutto, questo rilascio è relativo alla gestione dell'innovazione delle applicazioni. Nano Server o Nano come host è obsoleto e sostituito dal contenitore Nano, ovvero Nano eseguito come un'immagine del contenitore. 
...
**Replica di archiviazione**: la protezione del ripristino di emergenza aggiunta tramite Replica di archiviazione in Windows Server 2016 è ora espansa per includere:
...
>[!TIP]
>Se vuoi dare un'occhiata in anteprima alle altre straordinarie modifiche in arrivo per Servizi Desktop remoto, vedi [Servizi Desktop remoto: aggiornamenti e innovazioni future](https://blogs.technet.microsoft.com/enterprisemobility/2017/09/20/first-look-at-updates-coming-to-remote-desktop-services/)
...
**Rete Windows in parità con Linux per Kubernetes**: Windows è ora in parità con Linux in termini di rete. I clienti possono distribuire cluster Kubernetes con sistemi operativi misti in qualsiasi ambiente, inclusi Azure, in locale e in stack cloud di terzi con le stesse topologie e primitive di rete supportate su Linux, senza la necessità di eventuali soluzioni alternative o le estensioni del commutatore.

**Stack di rete core**: sono state migliorate numerose funzionalità dello stack di rete. Per altre informazioni su queste funzionalità, vedi la pagina relativa alle [funzionalità dello stack di rete core in Windows 10 Creators Update](https://blogs.technet.microsoft.com/networking/2017/07/13/core-network-stack-features-in-the-creators-update-for-windows-10/).
...
- **IPv6**: in questo rilascio sono disponibili numerosi miglioramenti in IPv6.
    - Supporto di **RFC 6106**: RFC 6106 consente la configurazione DNS tramite annunci router. Per abilitare o disabilitare il supporto di RFC 6106, puoi utilizzare il comando seguente:
...
- **Rilevamento Dead Gateway**: l'algoritmo di rilevamento Dead Gateway esegue automaticamente la transizione delle connessioni a un altro gateway quando il gateway corrente non è raggiungibile. In questo rilascio l'algoritmo è stato migliorato per eseguire di nuovo il probe dell'ambiente di rete periodicamente.
